 ...
 * Read-only view of inventory movements and fuel transactions
 */
$page_id = 'inv_history';
...
// ═══════════════════════════════════════════════════════════════════════════
$merch_movements = [];
$merch_stats = ['total' => 0, 'deliveries' => 0, 'releases' => 0];

try {
    // Fetch from actual merchandise_stock_in table (production data)
    $stmt = $pdo->prepare("
        SELECT 
            msi.encoded_at as date,
            msi.batch_ref as reference_no,
    ...
    $merch_movements = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Calculate stats from actual data
    $merch_stats['total'] = count($merch_movements);
    foreach ($merch_movements as $m) {
        if ($m['movement_type'] === 'Delivery') $merch_stats['deliveries']++;
        else $merch_stats['releases']++;
    }
} catch (Exception $e) {
...
// ═══════════════════════════════════════════════════════════════════════════
$fuel_transactions = [];
$fuel_stats = ['total' => 0, 'today' => 0, 'total_liters' => 0];

try {
    // Fetch from actual fuel_transactions table (production data)
    // Use helper function to determine shift based on transaction time
    $shift_case = get_shift_sql_case('ft.transaction_date');
    
    $stmt = $pdo->prepare("
        SELECT 
            ft.transaction_date as date,
            $shift_case as shift,
            ft.fuel_type as tank,
            ft.previous_reading as beginning,
            ft.present_reading as ending,
            ft.calibration as calibration,
            ft.liters_sold as dispensed,
            COALESCE(u.username, 'Unknown') as staff
    ...
    // Calculate stats from actual data
    $fuel_stats['total'] = count($fuel_transactions);
    $today = date('Y-m-d');
    foreach ($fuel_transactions as $f) {
        $date = date('Y-m-d', strtotime($f['date']));
        if ($date === $today) $fuel_stats['today']++;
        $fuel_stats['total_liters'] += (float)($f['dispensed'] ?? 0);
    }
} catch (Exception $e) {
...
.summary-card.sc-orange .sc-num { color: #ea580c; }
...
<div class="hist-tabs">
    <div class="hist-tab active" onclick="switchTab('merchandise')" id="tab-merchandise">
        <i class="fas fa-box"></i> Merchandise History
    </div>
    <div class="hist-tab" onclick="switchTab('fuel')" id="tab-fuel">
        <i class="fas fa-gas-pump"></i> Fuel History
    </div>
</div>
...
<div class="hist-tab-panel active" id="panel-merchandise">
    ...
    <div class="summary-grid">
        <div class="summary-card">
            <div class="sc-num"><?php echo number_format($merch_stats['total']); ?></div>
            <div class="sc-lbl">Total Movements</div>
        </div>
        <div class="summary-card sc-green">
            <div class="sc-num"><?php echo number_format($merch_stats['deliveries']); ?></div>
            <div class="sc-lbl">Deliveries</div>
        </div>
        <div class="summary-card sc-orange">
            <div class="sc-num"><?php echo number_format($merch_stats['releases']); ?></div>
            <div class="sc-lbl">Releases</div>
        </div>
    </div>
    ...
    <div class="hist-card">
        <div class="hist-card-body">
            <table class="hist-table">
                ...
                <tbody>
                <?php if (empty($merch_movements)): ?>
                    <tr>
                        <td colspan="6">
                            <div class="empty-state">
                                <i class="fas fa-inbox"></i>
                                <p style="margin:0;font-size:14px;">No merchandise inventory movements yet.</p>
                            </div>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($merch_movements as $m): ?>
                    <tr>
                        <td style="white-space:nowrap;">
                            <?php echo date('M d, Y g:i A', strtotime($m['date'])); ?>
                        </td>
                        <td>
                        ...
                        <td><strong><?php echo htmlspecialchars($m['product']); ?></strong></td>
                        <td>
                            <span style="display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:600;padding:3px 8px;border-radius:12px;background:#dcfce7;color:#166534;">
                                <i class="fas fa-arrow-down"></i> <?php echo htmlspecialchars($m['movement_type']); ?>
                            </span>
                        </td>
...
<div class="hist-tab-panel" id="panel-fuel">
    ...
    <div class="summary-grid">
        <div class="summary-card">
            <div class="sc-num"><?php echo number_format($fuel_stats['total']); ?></div>
            <div class="sc-lbl">Total Fuel Transactions</div>
        </div>
        <div class="summary-card sc-blue">
            <div class="sc-num"><?php echo number_format($fuel_stats['today']); ?></div>
            <div class="sc-lbl">Today's Transactions</div>
        </div>
        <div class="summary-card sc-green">
            <div class="sc-num"><?php echo number_format($fuel_stats['total_liters'], 2); ?></div>
            <div class="sc-lbl">Total Liters Dispensed</div>
        </div>
    </div>
                ...
                <?php if (empty($fuel_transactions)): ?>
                    <tr>
                        <td colspan="8">
                            <div class="empty-state">
                                <i class="fas fa-inbox"></i>
                                <p style="margin:0;font-size:14px;">No fuel transactions yet.</p>
                            </div>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($fuel_transactions as $f): ?>
                    <tr>
                        <td style="white-space:nowrap;">
                            <?php echo date('M d, Y', strtotime($f['date'])); ?>
                        </td>
                        <td>
                            <span style="font-size:11px;font-weight:600;padding:3px 8px;border-radius:12px;background:#fff3cd;color:#856404;">
                        ...
                        <td style="font-weight:700;color:#0284c7;">
                            <?php echo number_format((float)$f['dispensed'], 2); ?> L
                        </td>
                        <td><?php echo htmlspecialchars($f['staff'] ?? '—'); ?></td>
                    </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
                </tbody>
...
function switchTab(tab) {
    document.querySelectorAll('.hist-tab').forEach(el => el.classList.remove('active'));
    document.querySelectorAll('.hist-tab-panel').forEach(el => el.classList.remove('active'));
