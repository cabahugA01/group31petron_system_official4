...
// Database-Driven Multi-Service Manager
class DatabaseDrivenMultiServiceManager {
    constructor() {
    ...
    setupEventListeners() {
        ...
        serviceCheckboxes.forEach(checkbox => {
            checkbox.addEventListener('change', (e) => {
                ...
                if (isChecked) {
                    this.loadServicePartsFromDatabase(serviceName);
                } else {
                    this.removeService(serviceName);
                }
                
                this.updateDisplay();
                this.calculateTotalsWithDatabasePricing();
            });
        });
    }
    
    async loadServicePartsFromDatabase(serviceName) {
        try {
            // Show loading state
            ...
            // Load service data with database pricing
            const response = await fetch(`../backend/api/database_pricing.php?action=get_service_parts_with_pricing&service_key=${encodeURIComponent(serviceKey)}`);
            ...
            if (result.success && result.data) {
                ...
                this.selectedServices.set(serviceName, {
                    ...
                    labor_cost_per_hour: serviceData.labor_cost_per_hour,
                    ...
                    parts: serviceData.parts.map(part => ({
                        ...
                        from_inventory: part.from_inventory || false
                    }))
                });
    ...
    updateDisplay() {
        ...
        this.selectedServices.forEach((serviceData, serviceName) => {
            ...
            grandTotal += serviceData.total_cost;
            this.displayServiceSection(serviceName, serviceData);
        });
        
        if (autoText) {
            const serviceNames = Array.from(this.selectedServices.keys());
            autoText.textContent = `${this.selectedServices.size} services selected (${totalParts} total parts) - Grand Total: ₱${grandTotal.toFixed(2)}`;
        }
    }
    ...
    displayServiceSection(serviceName, serviceData) {
        ...
        // No service header - remove redundant labels since service types are already shown at the top
        ...
        serviceData.parts.forEach((part, index) => {
            const partDiv = this.createDatabaseDrivenPartElement(serviceName, part, index);
    ...
    createDatabaseDrivenPartElement(serviceName, part, index) {
        ...
        // Add visual indicator for inventory source
        const inventoryIcon = part.from_inventory ? 
            '<i class="fas fa-box" title="From Merchandise Inventory" style="color: #28a745;"></i>' : 
            '<i class="fas fa-database" title="Database Price" style="color: #17a2b8;"></i>';
        ...
        partDiv.innerHTML = `
            <div class="part-checkbox">
                <input type="checkbox" 
                       ...
                       data-from-inventory="${part.from_inventory}"
                       onchange="databaseServiceManager.togglePartSelection('${serviceName}', '${part.part_name}', this.checked)">
            </div>
            <div class="part-info">
                <label class="part-name">
                    <strong>${part.part_name}</strong>
                    <br><small class="part-category">${part.category || 'General'} - ₱${part.unit_price.toFixed(2)}/unit ${inventoryIcon}</small>
                </label>
            </div>
            <div class="part-quantity">
                <input type="number" 
                       ...
                       data-service="${serviceName}" 
                       data-part="${part.part_name}"
                       onchange="databaseServiceManager.updatePartQuantity('${serviceName}', '${part.part_name}', this.value)">
            </div>
            <div class="part-remarks">
                <input type="text" 
                       ...
                       data-service="${serviceName}" 
                       data-part="${part.part_name}"
                       onchange="databaseServiceManager.updatePartRemarks('${serviceName}', '${part.part_name}', this.value)">
            </div>
            <div class="part-cost">
            ...
            <div class="part-actions">
                <button type="button" 
                        ...
                        onclick="databaseServiceManager.removePart('${serviceName}', '${part.part_name}')"
                        title="Remove part">
    ...
    async calculateTotalsWithDatabasePricing() {
        try {
            const servicesData = [];
            ...
            this.selectedServices.forEach((serviceData, serviceName) => {
                const selectedParts = serviceData.parts
                    ...
                    .map(part => ({
                        ...
                        from_inventory: part.from_inventory
                    }));
            ...
            const response = await fetch('../backend/api/database_pricing.php?action=calculate_totals_with_real_pricing', {
                method: 'POST',
                headers: {
    ...
    displayDatabaseDrivenCostSummary(result) {
        ...
        if (!summaryContainer) {
            ...
            summaryContainer.className = 'cost-summary database-driven';
        ...
        summaryContainer.innerHTML = `
            <div class="cost-summary-header">
                <h3><i class="fas fa-calculator"></i> Cost Summary (Database Pricing)</h3>
                <div class="pricing-source">
                    <i class="fas fa-database"></i> All prices from merchandise inventory
                </div>
            </div>
    ...
    setupCostCalculation() {
        ...
        document.addEventListener('change', (e) => {
            if (e.target.matches('input[type="checkbox"][data-service]') ||
                ...
                setTimeout(() => this.calculateTotalsWithDatabasePricing(), 100);
            }
        });
    }
    
    // Verify all pricing is database-driven
    async verifyDatabasePricing() {
        try {
            const response = await fetch('../backend/api/database_pricing.php?action=verify_pricing_sources');
            const result = await response.json();
...
// Initialize database-driven multi-service manager
const databaseServiceManager = new DatabaseDrivenMultiServiceManager();

// Verify pricing on load
databaseServiceManager.verifyDatabasePricing();
