...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if (empty($full_name)) {
            $error_msg = 'Mechanic Name is required.';
            ...
            try {
                $stmt = $pdo->prepare("INSERT INTO mechanics (full_name, contact_no, address, specialization, status, station_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())");
                $stmt->execute([$full_name, $contact_no, $address, $specialization, $status, $station_id]);
...
<?php if (!empty($success_msg)): ?>
    <div style="background:#d1fae5; border:1px solid #a7f3d0; color:#065f46; padding:12px 16px; border-radius:8px; margin-bottom:20px; font-size:13px;">
...
<div class="filters-form">
    <div style="flex: 1; min-width: 200px;">
        <label>Search Mechanics</label>
        <input type="text" id="tableSearch" onkeyup="searchMechanicsTable()" class="inp" style="width: 100%;" placeholder="Search by name, specialty, contact...">
    </div>
</div>
                ...
                <?php if (empty($mechanics_list)): ?>
                    <tr>
                        <td colspan="6" style="text-align: center; padding: 40px; color: #94a3b8;">
                            ...
                            No mechanics registered. Click "Add New Mechanic" to begin.
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($mechanics_list as $row): 
                        $formattedId = sprintf("MEC-%03d", $row['id']);
                        $statusClass = $row['status'] === 'active' ? 'badge-active' : 'badge-inactive';
                    ?>
                        <tr data-id="<?= (int)$row['id'] ?>">
                            <td><strong><?= htmlspecialchars($formattedId) ?></strong></td>
                            <td class="mech-name"><strong><?= htmlspecialchars($row['full_name']) ?></strong></td>
                            <td class="mech-contact"><?= htmlspecialchars($row['contact_no'] ?: '—') ?></td>
                            <td class="mech-spec"><?= htmlspecialchars($row['specialization'] ?: 'General Mechanic') ?></td>
                            <td>
                            ...
                            <td style="vertical-align: middle;">
                                ...
                                <?php if ($row['status'] === 'active'): ?>
                                    <button class="tbl-btn deact" id="toggle-btn-<?= (int)$row['id'] ?>" onclick="toggleStatus(<?= (int)$row['id'] ?>, 'inactive')">
                                        <i class="fas fa-times"></i> Deactivate
                                    </button>
                                <?php else: ?>
                                    <button class="tbl-btn activ" id="toggle-btn-<?= (int)$row['id'] ?>" onclick="toggleStatus(<?= (int)$row['id'] ?>, 'active')">
...
<div id="viewModal" class="modal-backdrop">
    <div class="modal-content">
        <div class="modal-header">
            <span class="modal-title"><i class="fas fa-user"></i> Mechanic Details</span>
        ...
        <div class="modal-body" style="line-height: 1.6;">
            <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                <tr>
                    <td style="padding: 6px 0; color: #64748b; width: 35%;"><strong>Mechanic ID:</strong></td>
                    <td style="padding: 6px 0; color: #1e293b;" id="viewMecId">—</td>
                </tr>
                <tr>
                    <td style="padding: 6px 0; color: #64748b;"><strong>Full Name:</strong></td>
                    <td style="padding: 6px 0; color: #1e293b;" id="viewName">—</td>
                </tr>
                <tr>
                    <td style="padding: 6px 0; color: #64748b;"><strong>Contact Number:</strong></td>
                    <td style="padding: 6px 0; color: #1e293b;" id="viewContact">—</td>
                </tr>
                <tr>
                    <td style="padding: 6px 0; color: #64748b;"><strong>Address:</strong></td>
                    <td style="padding: 6px 0; color: #1e293b;" id="viewAddress">—</td>
                </tr>
                <tr>
                    <td style="padding: 6px 0; color: #64748b;"><strong>Specialty:</strong></td>
                    <td style="padding: 6px 0; color: #1e293b;" id="viewSpecialty">—</td>
                </tr>
                <tr>
                    <td style="padding: 6px 0; color: #64748b;"><strong>Status:</strong></td>
                    <td style="padding: 6px 0;" id="viewStatusContainer">—</td>
                </tr>
                <tr>
        ...
        <div class="modal-footer">
            <button onclick="closeViewModal()" style="
                ...
                padding: 0 16px; height: 36px; border-radius: 7px;
                font-size: 13px; font-weight: 600; cursor: pointer; transition: all .15s;"
                onmouseover="this.style.background='#6b7280';this.style.color='#fff';"
                onmouseout="this.style.background='white';this.style.color='#4b5563';">Close</button>
        </div>
    </div>
</div>
...
<div id="addEditModal" class="modal-backdrop">
    <div class="modal-content">
        <form method="POST" id="mechanicForm">
            ...
            <div class="modal-header">
                <span class="modal-title" id="modalTitle"><i class="fas fa-plus"></i> Add New Mechanic</span>
            ...
            <div class="modal-body">
                <div class="form-field">
                    <label>Mechanic Name <span style="color:#dc2626;">*</span></label>
                    <input type="text" name="full_name" id="field_name" required placeholder="Enter mechanic full name...">
                </div>
                <div class="form-field">
                    <label>Contact Number <span style="color:#dc2626;">*</span></label>
                    <input type="text" name="contact_no" id="field_contact" required placeholder="Enter 11-digit mobile number or landline...">
                </div>
                <div class="form-field">
                    <label>Address</label>
                    <textarea name="address" id="field_address" placeholder="Enter residential address (optional)..."></textarea>
                </div>
                <div class="form-field">
                ...
                <div class="form-field">
                    ...
                    <div style="display:flex; gap:16px; margin-top:4px;">
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer; font-weight:normal;">
                            <input type="radio" name="status" id="status_active" value="active" checked> Active
                        </label>
                        <label style="display:flex; align-items:center; gap:6px; cursor:pointer; font-weight:normal;">
                            <input type="radio" name="status" id="status_inactive" value="inactive"> Inactive
                        </label>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" onclick="closeAddEditModal()" style="
                    background: white; color: #4b5563; border: 1px solid #6b7280;
                    display: inline-flex; align-items: center; gap: 6px;
                    padding: 0 16px; height: 36px; border-radius: 7px;
                    font-size: 13px; font-weight: 600; cursor: pointer; transition: all .15s;"
                    onmouseover="this.style.background='#6b7280';this.style.color='#fff';"
                    onmouseout="this.style.background='white';this.style.color='#4b5563';">Cancel</button>
                <button type="submit" id="btnSave" style="
                    background: #002F70; color: #fff; border: 1px solid #002F70;
                    display: inline-flex; align-items: center; gap: 6px;
                    padding: 0 16px; height: 36px; border-radius: 7px;
                    font-size: 13px; font-weight: 600; cursor: pointer; transition: all .15s;"
                    onmouseover="this.style.background='#001f4d';this.style.borderColor='#001f4d';"
                    onmouseout="this.style.background='#002F70';this.style.borderColor='#002F70';"><i class="fas fa-save"></i> Save</button>
            </div>
        </form>
    </div>
</div>
...
function searchMechanicsTable() {
    ...
    for (let i = 1; i < tr.length; i++) {
        const tdName = tr[i].getElementsByClassName('mech-name')[0];
        const tdContact = tr[i].getElementsByClassName('mech-contact')[0];
        const tdSpec = tr[i].getElementsByClassName('mech-spec')[0];
        
        if (tdName || tdContact || tdSpec) {
            const txtName = (tdName ? tdName.textContent : '');
            const txtContact = (tdContact ? tdContact.textContent : '');
            const txtSpec = (tdSpec ? tdSpec.textContent : '');
            
            if (txtName.toLowerCase().indexOf(filter) > -1 || 
                txtContact.toLowerCase().indexOf(filter) > -1 || 
                txtSpec.toLowerCase().indexOf(filter) > -1) {
                tr[i].style.display = "";
            } else {
...
function openViewModal(mec) {
    const formattedId = "MEC-" + String(mec.id).padStart(3, '0');
    ...
    document.getElementById('viewAddress').textContent = mec.address || '—';
    document.getElementById('viewSpecialty').textContent = mec.specialization || 'General Mechanic';
    
    const badgeClass = mec.status === 'active' ? 'badge-active' : 'badge-inactive';
    const statusText = mec.status.charAt(0).toUpperCase() + mec.status.slice(1);
    document.getElementById('viewStatusContainer').innerHTML = `<span class="badge ${badgeClass}">${statusText}</span>`;
...
function openAddModal() {
    ...
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-plus"></i> Add New Mechanic';
...
function openEditModal(mec) {
    ...
    document.getElementById('modalTitle').innerHTML = '<i class="fas fa-edit"></i> Edit Mechanic';
    
    // Populate fields
    document.getElementById('field_name').value = mec.full_name;
    document.getElementById('field_contact').value = mec.contact_no || '';
    document.getElementById('field_address').value = mec.address || '';
    document.getElementById('field_specialty').value = mec.specialization || 'General Mechanic';
    
    if (mec.status === 'active') {
        document.getElementById('status_active').checked = true;
    } else {
...
async function toggleStatus(id, newStatus) {
    ...
    try {
        ...
        if (result.success) {
            ...
            if (btn) {
                if (newStatus === 'active') {
                    btn.className = 'tbl-btn deact';
                    btn.innerHTML = '<i class="fas fa-times"></i> Deactivate';
                    btn.setAttribute('onclick', `toggleStatus(${id}, 'inactive')`);
                } else {
                    btn.className = 'tbl-btn activ';
                    btn.innerHTML = '<i class="fas fa-check"></i> Activate';
                    btn.setAttribute('onclick', `toggleStatus(${id}, 'active')`);
                }
            }
            
            // Reload window to refresh KPI counts and JSON data representation cleanly
