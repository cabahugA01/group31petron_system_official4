    ...
    switch ($_POST['action']) {
        ...
        case 'validate_reading':
            ...
            try {
                ...
                $stmt = $pdo->prepare("SELECT ft.*, COALESCE(NULLIF(CONCAT(TRIM(COALESCE(u.first_name,'')), ' ', TRIM(COALESCE(u.last_name,''))), ' '), u.username, 'Unknown') as staff_name FROM fuel_transactions ft JOIN users u ON ft.staff_id=u.id WHERE ft.transaction_id=? AND ft.station_id=?");
            ...
            header('Location: manager_fuel_transactions.php'); exit;
        ...
        case 'validate_delivery':
            ...
            try {
                ...
                $stmt = $pdo->prepare("
                    SELECT fd.*, COALESCE(NULLIF(CONCAT(TRIM(COALESCE(u.first_name,'')), ' ', TRIM(COALESCE(u.last_name,''))), ' '), u.username, 'Unknown') AS staff_name
...
try {
    $stmt = $pdo->prepare("SELECT fp.*, ft.name as fuel_type_name, COALESCE(NULLIF(CONCAT(TRIM(COALESCE(u.first_name,'')), ' ', TRIM(COALESCE(u.last_name,''))), ' '), u.username, 'Unknown') as updated_by_name FROM fuel_pumps fp JOIN fuel_types ft ON fp.fuel_type_id=ft.id LEFT JOIN users u ON fp.calibration_updated_by=u.id WHERE fp.station_id=? ORDER BY COALESCE(fp.calibration_updated_at,fp.created_at) DESC LIMIT 10");
    $stmt->execute([$station_id]);
...
try {
    $stmt = $pdo->prepare("
        SELECT fvr.*, COALESCE(NULLIF(CONCAT(TRIM(COALESCE(u.first_name,'')), ' ', TRIM(COALESCE(u.last_name,''))), ' '), u.username, 'Unknown') as resolved_by_name
        FROM fuel_variance_reports fvr
        LEFT JOIN users u ON fvr.resolved_by = u.id
...
try {
    $stmt = $pdo->prepare("SELECT fa.*, ft.name as fuel_type_name, COALESCE(NULLIF(CONCAT(TRIM(COALESCE(u.first_name,'')), ' ', TRIM(COALESCE(u.last_name,''))), ' '), u.username, 'Unknown') as user_name FROM fuel_adjustments fa JOIN fuel_types ft ON fa.fuel_type_id=ft.id JOIN users u ON fa.user_id=u.id WHERE fa.station_id=? ORDER BY fa.adjustment_date DESC, fa.created_at DESC LIMIT 15");
    $stmt->execute([$station_id]);
...
try {
    $stmt = $pdo->prepare("
        SELECT fd.*, 
               COALESCE(NULLIF(CONCAT(TRIM(COALESCE(u.first_name,'')), ' ', TRIM(COALESCE(u.last_name,''))), ' '), u.username, 'Unknown') as recorded_by_name, 
               COALESCE(NULLIF(CONCAT(TRIM(COALESCE(v.first_name,'')), ' ', TRIM(COALESCE(v.last_name,''))), ' '), v.username, '—') as verified_by_name,
...
try {
    $stmt = $pdo->prepare("
        SELECT
            ...
            COALESCE(NULLIF(CONCAT(TRIM(COALESCE(u.first_name,'')), ' ', TRIM(COALESCE(u.last_name,''))), ' '), u.username, 'Unknown') AS calibration_encoded_by
        FROM fuel_inventory fi
        LEFT JOIN fuel_pumps fp
...
.page-head { display:flex; align-items:flex-start; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-top:0 !important; margin-bottom:20px; padding-top:0 !important; padding-bottom:16px; border-bottom:2px solid #e9ecef; }
...
<div class="mfm-alert <?php echo $msg_type; ?>">
    ...
    .mfm-tab-btn .badge { background:#ffc107; color:#212529; font-size:.7rem; padding:2px 6px; border-radius:10px; font-weight:800; }
    .fuel-section { display:none; animation:fadeIn 0.3s ease; }
    .fuel-section.active { display:block; }
...
.audit-badge, .tag-open, .tag-investigate, .tag-resolved {
    ...
    <div class="mfm-tabs">
        <button class="mfm-tab-btn active" onclick="switchMfmTab('tab-validations', this)"><i class="fas fa-clipboard-check"></i> Pending Validations <?php if(($pending_cnt??0) + ($pending_daily_cnt??0) > 0): ?><span class="badge"><?php echo ($pending_cnt??0) + ($pending_daily_cnt??0); ?></span><?php endif; ?></button>
        <button class="mfm-tab-btn" onclick="switchMfmTab('tab-reconciliation', this)"><i class="fas fa-balance-scale"></i> Reconciliation &amp; Variance</button>
        <button class="mfm-tab-btn" onclick="switchMfmTab('tab-history', this)"><i class="fas fa-history"></i> Audit History</button>
        <button class="mfm-tab-btn" onclick="switchMfmTab('tab-reports', this)"><i class="fas fa-chart-bar"></i> Sales Summary</button>
    </div>

    <script>
    function switchMfmTab(tabId, btn) {
        document.querySelectorAll('.fuel-section').forEach(el => el.classList.remove('active'));
        document.querySelectorAll('.mfm-tab-btn').forEach(el => el.classList.remove('active'));
        
        if(tabId === 'tab-validations') {
            document.getElementById('fuel-transactions').classList.add('active');
            document.getElementById('daily-ops').classList.add('active');
        } else if(tabId === 'tab-reconciliation') {
            document.getElementById('reconciliation').classList.add('active');
            document.getElementById('variance-reports').classList.add('active');
        } else if(tabId === 'tab-history') {
            document.getElementById('shift-history').classList.add('active');
        } else if(tabId === 'tab-reports') {
            document.getElementById('fuel-reports').classList.add('active');
        }
        
        btn.classList.add('active');
    }
    
    // Quick script to auto-switch tab if hash is present
    document.addEventListener("DOMContentLoaded", function() {
        if(window.location.hash) {
            let h = window.location.hash.substring(1);
            if(h === 'reconciliation' || h === 'variance-reports') {
                document.querySelector('[onclick="switchMfmTab(\'tab-reconciliation\', this)"]').click();
            } else if(h === 'shift-history') {
                document.querySelector('[onclick="switchMfmTab(\'tab-history\', this)"]').click();
            } else if(h === 'fuel-reports') {
                document.querySelector('[onclick="switchMfmTab(\'tab-reports\', this)"]').click();
            }
        }
    });
    </script>
...
---------------------------------------------------------- -->
<div id="fuel-transactions" class="fuel-section active">
...
---------------------------------------------------------- -->
<div id="daily-ops" class="fuel-section active">
<div class="fuel-section-inner">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('a[href*="manager_fuel_transactions.php"]').forEach(function(link) {
