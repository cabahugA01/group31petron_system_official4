...
try {
    $pdo->exec("CREATE TABLE IF NOT EXISTS deliveries_oversight (
        ...
        status ENUM('Pending Validation','Validated','Flagged') NOT NULL DEFAULT 'Pending Validation',
...
// ── POST: Encode new delivery ──────────────────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'encode_delivery') {
    $delivery_type = in_array($_POST['delivery_type'] ?? '', ['fuel', 'merchandise'])
        ? $_POST['delivery_type']
        : 'fuel';
...
try {
    $stmt = $pdo->prepare("
        SELECT d.*, u.name AS encoded_by_name
        FROM deliveries_oversight d
        LEFT JOIN users u ON d.encoded_by = u.id
        WHERE d.encoded_by = ? AND d.station_id = ?
...
.step-arrow { margin: 0 12px; color: #cbd5e1; font-size: 14px; flex-shrink: 0; }
...
.enc-alert i { margin-top: 1px; flex-shrink: 0; }
...
.form-section-title { font-size: 11px; font-weight: 700; color: var(--gray); text-transform: uppercase; letter-spacing: .6px; margin: 18px 0 10px; padding-bottom: 6px; border-bottom: 1px solid #f0f0f0; }
...
table.enc-dt tr:hover td { background: #f8fafc; }
...
<!-- Page Header -->
<div class="enc-page-head">
    <h1><i class="fas fa-truck-loading" style="color:var(--blue);"></i> Encode Delivery Record</h1>
    <p>Submit a new delivery record for Admin validation. Station: <strong><?php echo htmlspecialchars($station_name); ?></strong></p>
</div>


<!-- Alert Banner -->
<?php if ($msg): ?>
<div class="enc-alert enc-alert-<?php echo $msg_type === 'success' ? 'success' : 'error'; ?>">
...
<div class="info-box">
    ...
    <span>Delivery records you encode here will appear in the <strong>Admin's Deliveries Oversight</strong> page with status <strong>Pending Validation</strong>. The Admin will review and validate or flag each entry.</span>
</div>

<!-- Encode Form Card -->
<div class="enc-card">
    <div class="enc-card-head">
        <h3 class="enc-card-title"><i class="fas fa-plus-circle"></i> New Delivery Record</h3>
        <span style="font-size:12px;color:var(--gray);">Encoded by: <strong><?php echo htmlspecialchars($me['name'] ?? $me['username'] ?? 'Manager'); ?></strong></span>
    </div>
    <div class="enc-card-body">
        <form method="POST" action="" id="encodeForm">
            <input type="hidden" name="action" value="encode_delivery">

            <div class="form-section-title"><i class="fas fa-tag"></i> Delivery Classification</div>
            <div class="form-grid-2">
                <div class="form-group">
                    <label>Delivery Type <span class="req">*</span></label>
                    <select name="delivery_type" id="deliveryType" onchange="updateUnitOptions()">
                        <option value="fuel"          <?php echo ($_POST['delivery_type'] ?? '') === 'fuel'          ? 'selected' : ''; ?>>Fuel</option>
                        <option value="merchandise"   <?php echo ($_POST['delivery_type'] ?? '') === 'merchandise'   ? 'selected' : ''; ?>>Merchandise</option>
                    </select>
                </div>
                <div class="form-group">
                    <label>DR Number <span style="color:var(--gray);font-weight:400;">(optional)</span></label>
                    <input type="text" name="dr_number" placeholder="e.g. DR-2026-00123"
                           value="<?php echo htmlspecialchars($_POST['dr_number'] ?? ''); ?>">
                </div>
            </div>

            <div class="form-section-title"><i class="fas fa-box"></i> Product Details</div>
            <div class="form-grid-2">
                <div class="form-group">
                    <label>Supplier <span class="req">*</span></label>
                    <input type="text" name="supplier" placeholder="e.g. Petron Corporation"
                           value="<?php echo htmlspecialchars($_POST['supplier'] ?? ''); ?>" required>
                </div>
                <div class="form-group">
                    <label>Product <span class="req">*</span></label>
                    <input type="text" name="product" placeholder="e.g. Diesel, Unleaded 95, Motor Oil"
                           value="<?php echo htmlspecialchars($_POST['product'] ?? ''); ?>" required>
                </div>
            </div>
            <div class="form-grid-3">
                <div class="form-group">
                    <label>Quantity <span class="req">*</span></label>
                    <input type="number" name="quantity" placeholder="e.g. 5000" min="0.001" step="0.001"
                ...
                <div class="form-group">
                    ...
                    <select name="unit" id="unitSelect">
                        <option value="L"     <?php echo ($_POST['unit'] ?? 'L') === 'L'     ? 'selected' : ''; ?>>Liters (L)</option>
                        ...
                        <option value="drums" <?php echo ($_POST['unit'] ?? '') === 'drums'  ? 'selected' : ''; ?>>Drums</option>
                ...
                <div class="form-group">
                    ...
                    <input type="date" name="delivery_date"
                           value="<?php echo htmlspecialchars($_POST['delivery_date'] ?? date('Y-m-d')); ?>" required>
                </div>
            </div>

            <div class="form-section-title"><i class="fas fa-sticky-note"></i> Additional Notes</div>
            <div class="form-group">
                <label>Notes <span style="color:var(--gray);font-weight:400;">(optional)</span></label>
                <textarea name="notes" placeholder="e.g. Delivery received in good condition. Verified against physical DR."><?php echo htmlspecialchars($_POST['notes'] ?? ''); ?></textarea>
            </div>

            <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:20px;padding-top:16px;border-top:1px solid #f0f0f0;">
                <a href="manager_dashboard.php" class="btn btn-outline"><i class="fas fa-arrow-left"></i> Back to Dashboard</a>
                <button type="submit" class="btn btn-primary" id="submitBtn">
                    <i class="fas fa-paper-plane"></i> Submit for Admin Validation
                </button>
            </div>
        </form>
    </div>
...
<div class="enc-card">
    <div class="enc-card-head">
        <h3 class="enc-card-title"><i class="fas fa-history"></i> My Recent Deliveries <span style="font-size:12px;font-weight:400;color:var(--gray);">(Last 30 days)</span></h3>
        <span style="font-size:12px;color:var(--gray);"><?php echo count($recent_deliveries); ?> record(s)</span>
    </div>
    <div class="enc-card-body" style="padding:0;">
        <div class="enc-table-wrap">
            <?php if (empty($recent_deliveries)): ?>
            <div class="empty-state">
                <i class="fas fa-truck"></i>
                No delivery records found in the last 30 days.<br>
                <small>Use the form above to encode your first delivery.</small>
            </div>
            <?php else: ?>
            <table class="enc-dt">
                <thead>
                ...
                <tbody>
                    ...
                    <?php
                        ...
                        $typeClass = $d['delivery_type'] === 'fuel' ? 'badge-fuel' : 'badge-merch';
                        $typeLabel = $d['delivery_type'] === 'fuel' ? 'Fuel' : 'Merchandise';
                        $qty = number_format((float)$d['quantity'], 2) . ' ' . htmlspecialchars($d['unit']);
                    ?>
                    <tr>
                        <td><?php echo (int)$d['id']; ?></td>
                        <td style="font-size:11px;color:var(--gray);font-family:monospace;"><?php echo htmlspecialchars($d['delivery_ref']); ?></td>
                        <td><span class="badge <?php echo $typeClass; ?>"><?php echo $typeLabel; ?></span></td>
                        <td><?php echo htmlspecialchars($d['dr_number'] ?: '—'); ?></td>
                        <td><?php echo htmlspecialchars($d['supplier']); ?></td>
                        <td><?php echo htmlspecialchars($d['product']); ?></td>
                        <td><?php echo $qty; ?></td>
                        <td><?php echo htmlspecialchars(date('M j, Y', strtotime($d['delivery_date']))); ?></td>
                        <td><span class="badge <?php echo $statusClass; ?>"><?php echo htmlspecialchars($d['status']); ?></span></td>
                        <td style="font-size:11px;color:var(--gray);"><?php echo htmlspecialchars(date('M j, Y g:i A', strtotime($d['created_at']))); ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
...
function updateUnitOptions() {
    ...
    const fuelUnits = ['L', 'drums'];
    const merchUnits = ['pcs', 'kg', 'boxes', 'cans', 'L'];

    const units = type === 'fuel' ? fuelUnits : merchUnits;
    const labels = {
        'L': 'Liters (L)', 'drums': 'Drums', 'pcs': 'Pieces (pcs)',
        'kg': 'Kilograms (kg)', 'boxes': 'Boxes', 'cans': 'Cans'
    };
...
document.getElementById('encodeForm').addEventListener('submit', function() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Submitting&hellip;';
});
...
document.addEventListener('DOMContentLoaded', function() {
    const savedUnit = '<?php echo $savedUnit; ?>';
    const unitSelect = document.getElementById('unitSelect');
    for (let i = 0; i < unitSelect.options.length; i++) {
        if (unitSelect.options[i].value === savedUnit) {
