...
/* ── POST: Admin finalizes PO → Official ── */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'finalize_po') {
    ...
    if ($po_id > 0 && $final_qty > 0 && $final_price >= 0) {
        try {
            ...
            // Audit trail on linked stock request
            if (!empty($po_record['request_id'])) {
                $pdo->prepare("
                    ...
                    VALUES (?, 'PO Finalized', ?, ?, 'Validated', 'Official', ?)
                ")->execute([
                    $po_record['request_id'],
                    $me['id'],
                    $role,
                    "Admin finalized PO: {$po_record['po_number']}. Final Qty: {$final_qty}, Unit Price: ₱{$final_price}, Total: ₱{$total_amount}. Admin: {$me['name']}"
                ]);
            }

            log_activity($pdo, $me['id'], 'Finalize Purchase Order',
                "PO {$po_record['po_number']} finalized as Official. {$po_record['product_name']} | Qty: {$final_qty} | Total: ₱{$total_amount}");

            $pdo->commit();

            $msg     = "&#10003; PO <strong>{$po_record['po_number']}</strong> finalized as <strong>Official</strong>. Ready for supplier.";
            $msgType = 'success';
...
try {
    $rows = $pdo->query("
        SELECT po.*,
               ...
               sr.approved_quantity  AS sr_validated_qty,
...
.btn-finalize { background:#002F70; color:#fff; border:none; padding:6px 14px; border-radius:6px; cursor:pointer; font-size:12px; font-weight:600; display:inline-flex; align-items:center; gap:5px; }
.btn-finalize:hover { background:#001F4F; }

/* Audit trail chain */
.audit-chain { display:flex; align-items:center; gap:6px; font-size:11px; color:#6c757d; flex-wrap:wrap; }
...
.modal-box { background:#fff; border-radius:14px; padding:28px; width:540px; max-width:calc(100vw - 32px); max-height:calc(100vh - 40px); overflow-y:auto; box-shadow:0 24px 80px rgba(0,0,0,.3); position:relative; z-index:10000; animation:modalIn .2s ease; }
...
.modal-close { background:none; border:none; font-size:22px; cursor:pointer; color:#adb5bd; }
...
.field-group input:focus { border-color:#002F70; outline:none; box-shadow:0 0 0 3px rgba(0,47,112,.12); }
.modal-footer { display:flex; gap:10px; justify-content:flex-end; margin-top:18px; padding-top:14px; border-top:1px solid #e9ecef; }
.total-preview { background:#e8f4fd; border-left:4px solid #002F70; border-radius:6px; padding:10px 14px; margin-bottom:14px; font-size:14px; color:#002F70; font-weight:700; }
</style>

<div class="page-head">
    <div>
        <h1 class="h1"><i class="fas fa-file-invoice"></i> Purchase Orders</h1>
        <div class="sub">Auto-generated from Manager-validated stock requests &mdash; Admin finalizes for supplier</div>
    </div>
...
<div class="po-card">
    <div class="po-card-head">
        <div class="po-card-title">
            <i class="fas fa-clock"></i> Pending Admin Validation
        ...
        <span style="font-size:12px;color:#6c757d;">Review the auto-generated PO, set final qty &amp; price, then finalize as Official.</span>
    </div>
    <div class="po-card-body">
        <?php if (empty($pos_pending)): ?>
            <div style="text-align:center;padding:36px;color:#6c757d;">
                <i class="fas fa-check-circle" style="font-size:2.5em;display:block;margin-bottom:10px;color:#28a745;opacity:.5;"></i>
                No POs pending validation.
            </div>
        <?php else: ?>
        <div class="table-wrap">
            <table class="table">
                ...
                <tr>
                    ...
                    <td style="text-align:center;font-weight:700;"><?php echo number_format($po['quantity'], 0); ?></td>
                    <td>&#8369;<?php echo number_format($po['unit_price'], 2); ?></td>
                    ...
                    <td>
                        <button class="btn-finalize" onclick="openFinalize(
                            ...
                            <?php echo (float)$po['quantity']; ?>,
                            <?php echo (float)$po['unit_price']; ?>
                        )">
...
<div class="po-card">
    <div class="po-card-head">
        ...
        <span style="font-size:12px;color:#6c757d;"><?php echo count($pos_processed); ?> records</span>
    </div>
    <div class="po-card-body">
        ...
        <div class="table-wrap">
            <table class="table">
                ...
                <?php foreach ($pos_processed as $po):
                    $st_key = strtolower(str_replace(' ', '-', $po['status'] ?? ''));
                ...
                <tr>
                    ...
                    <td style="text-align:center;"><?php echo number_format($po['quantity'], 0); ?></td>
                    <td>&#8369;<?php echo number_format($po['unit_price'], 2); ?></td>
                    <td><strong>&#8369;<?php echo number_format($po['total_amount'], 2); ?></strong></td>
                    <td><span class="<?php echo $cls; ?>"><?php echo htmlspecialchars($po['status']); ?></span></td>
                    <td style="font-size:12px;"><?php echo htmlspecialchars($po['approved_by_name'] ?? '—'); ?></td>
                    <td style="font-size:12px;color:#6c757d;"><?php echo $po['approved_at'] ? date('M d, Y', strtotime($po['approved_at'])) : date('M d, Y', strtotime($po['created_at'])); ?></td>
                    <td>
                        <a href="print_po_new.php?id=<?php echo (int)$po['id']; ?>" target="_blank"
                           ...
                           title="Open official PO document for printing">
...
<div class="modal-overlay" id="finalizeModal">
    <div class="modal-box">
        <div class="modal-head">
            <div class="modal-title"><i class="fas fa-check-double" style="color:#28a745;"></i> Finalize Purchase Order</div>
        ...
        <form method="POST" id="finalizeForm">
            ...
            <div style="background:#e8f4fd;border-left:4px solid #002F70;border-radius:6px;padding:10px 14px;margin-bottom:14px;font-size:12px;color:#002F70;line-height:1.6;">
                <i class="fas fa-info-circle"></i>
                <strong>Finalizing this PO:</strong><br>
                &bull; Status → <strong>Official</strong> (ready for supplier)<br>
                &bull; Audit trail updated: Validated → Official<br>
                &bull; Activity log recorded with Admin ID &amp; timestamp
            </div>
            <div class="modal-footer">
                <button type="button" onclick="closeFinalize()" style="padding:9px 22px;background:#6c757d;color:#fff;border:none;border-radius:6px;cursor:pointer;">Cancel</button>
                <button type="submit" style="padding:9px 22px;background:#002F70;color:#fff;border:none;border-radius:6px;cursor:pointer;font-weight:600;">
                    <i class="fas fa-check-double"></i> Finalize as Official
                </button>
            </div>
        </form>
    </div>
...
function openFinalize(id, poNum, product, qty, price) {
    ...
    document.getElementById('finQty').value      = qty;
...
function computeTotal() {
    ...
    document.getElementById('finTotal').textContent = (q * p).toFixed(2);
}
