    ...
    if (empty($recovery_id)) {
        ...
        try {
            ...
            if ($detected_type === 'email') {
                $sql = "SELECT `{$uid_col}` AS user_id, username, email FROM users WHERE email = ? AND status = ? LIMIT 1";
            } else {
                $sql = "SELECT `{$uid_col}` AS user_id, username, email FROM users WHERE username = ? AND status = ? LIMIT 1";
            }
            ...
            if ($user) {
                if (empty($user['email'])) {
                    ...
                    $pdo->prepare("INSERT INTO password_reset_tokens (user_id, token, token_type, expires_at, ip_address) VALUES (?, ?, 'reset', ?, ?)")
                        ->execute([$user['user_id'], $otp_code, $expiry, $_SERVER['REMOTE_ADDR']]);
