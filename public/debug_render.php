...
try {
    ...
    echo "Pre-fetch Error: " . $e->getMessage() . "\n";
}
...
foreach ($voided as $v) {
    ...
    // Paste identical display code from public/voided_transactions.php
    ...
    if (!empty($v_fields['voided_items'])) {
        foreach ($v_fields['voided_items'] as $item) {
            ...
            echo '  [snapshot] ' . $item['product_name'] . ' - ' . $qty . ' x ₱' . number_format($price, 2) . ' = ₱' . number_format($sub, 2) . "\n";
        }
    } elseif (!empty($void_items_map[$txn_id])) {
        foreach ($void_items_map[$txn_id] as $item) {
            $is_svc = ($item['item_type'] === 'service');
            $icon   = $is_svc ? '🔧' : '📦';
            $qty    = isset($item['quantity'])   ? (float)$item['quantity']   : 1;
            $price  = isset($item['unit_price']) ? (float)$item['unit_price'] : 0;
            $sub    = isset($item['subtotal'])   ? (float)$item['subtotal']   : 0;
            echo '  [live] ' . $icon . ' ' . $item['product_name'] . ' - Qty: ' . $qty . ' x ₱' . number_format($price, 2) . ' = ₱' . number_format($sub, 2) . "\n";
        }
    } else {
        $amt = (float)$v['amount'];
        echo '  [fallback] ₱' . number_format($amt, 2) . " voided (Item details not available – legacy record)\n";
    }
    echo "---------------------------\n";
}
?>
