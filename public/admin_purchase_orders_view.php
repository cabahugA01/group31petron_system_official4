...
/* Override global .main padding so this page is truly full-width */
.main, .main-content {
  padding: 0 !important;
}
...
<div class="po-container">
  ...
  <div class="po-cards-grid">
    ...
    <div class="po-card card-total" id="card-total" onclick="setStatusTab('completed')">
      <div class="po-card-info">
        <span class="po-card-label" id="lbl-total-card">Total Purchase Orders</span>
        <span class="po-card-val" id="val-completed">0</span>
      </div>
      <div class="po-card-icon"><i class="fas fa-gas-pump"></i></div>
    </div>
  </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Restore from localStorage if not explicitly set in URL query string
  const urlParams = new URLSearchParams(window.location.search);
  if (!urlParams.has('type')) {
    const savedSubTab = localStorage.getItem('admin_po_active_subtab');
    if (savedSubTab && (savedSubTab === 'merch' || savedSubTab === 'fuel')) {
      activeSubTab = savedSubTab;
    }
  }
  if (!urlParams.has('tab')) {
    const savedStatusTab = localStorage.getItem('admin_po_active_status_tab');
    if (savedStatusTab && ['pending', 'approved', 'waiting', 'completed'].includes(savedStatusTab)) {
      activeStatusTab = savedStatusTab;
    }
  }
  ...
  if (activeSubTab === 'fuel') {
    ...
    document.getElementById('subtab-fuel').classList.remove('active');
  }
  
  // Apply correct table headers and card labels for initial subtab
  updateTableHead();
  updateCardLabels();
...
function setSubTab(type) {
  ...
  renderSummaryCounts();
  applyFilters();
  // Persist tab state in URL so refresh stays on same tab
  const url = new URL(window.location.href);
  url.searchParams.set('type', type);
  url.searchParams.set('tab', activeStatusTab);
  history.replaceState(null, '', url.toString());
  try { localStorage.setItem('admin_po_active_subtab', type); } catch(e) {}
}
...
function updateTableHead() {
  ...
  if (activeSubTab === 'fuel') {
    head.innerHTML = `<tr>
      <th>PO No.</th><th>PR No.</th><th>Generated By</th><th>Date</th>
      <th style="text-align:center;">Fuel Types</th><th>Status</th>
    </tr>`;
  } else {
    head.innerHTML = `<tr>
      <th>PO No.</th><th>PR No.</th><th>Generated By</th><th>Date</th>
      <th style="text-align:center;">Products</th><th>Status</th>
    </tr>`;
  }
}

function updateCardLabels() {
  const lbl = document.getElementById('lbl-total-card');
  if (lbl) lbl.textContent = activeSubTab === 'fuel' ? 'Total Fuel Purchase Orders' : 'Total Purchase Orders';
  document.querySelectorAll('.po-card-label').forEach((el, i) => {
    if (i === 0) el.textContent = activeSubTab === 'fuel' ? 'Pending Fuel POs' : 'Pending Purchase Orders';
    if (i === 1) el.textContent = activeSubTab === 'fuel' ? 'Approved Fuel POs' : 'Approved Purchase Orders';
    if (i === 2) el.textContent = activeSubTab === 'fuel' ? 'Printed Fuel POs' : 'Printed Purchase Orders';
  });
}
...
function setStatusTab(tab) {
  ...
  // Persist tab state in URL so refresh stays on same tab
  const url = new URL(window.location.href);
  url.searchParams.set('type', activeSubTab);
  url.searchParams.set('tab', tab);
  history.replaceState(null, '', url.toString());
  try { localStorage.setItem('admin_po_active_status_tab', tab); } catch(e) {}
}
...
// Safe data store – avoids JSON-in-onclick escaping issues
const _rowStore = {};
let _rowStoreIdx = 0;
...
function renderTable(data) {
  ...
  tbody.innerHTML = '';
  const isFuel = activeSubTab === 'fuel';
  ...
  order.forEach(d => {
    ...
    // PO Nos – collect unique non-null values
    ...
    // PR Nos – collect all unique values
    ...
    // Generated by – first non-empty value
    ...
    const products = isFuel ? (merged.total_items + ' Fuel Type(s)') : (merged.total_items + ' Product(s)');
...
function toggleRowDetails(key) {
  ...
  const allPRs = (r._all_pr_nos && r._all_pr_nos.length > 1 ? r._all_pr_nos.join(', ') : r.pr_no) || '—';
    ...
    detailsHtml = `
      <form method="POST" action="admin_purchase_orders.php" id="poForm-${key}">
        ...
        <div class="po-details-card">
          ...
          <p class="po-card-section-title" style="margin-top:20px;"><i class="fas fa-gas-pump"></i> ${activeSubTab === 'fuel' ? 'Ordered Fuel' : 'Ordered Products'}</p>
          <div class="po-products-wrap">
            <table class="po-products-table">
              <thead id="prod-thead-${key}">
                <tr>
                  ${activeSubTab === 'fuel'
              ...
              <tbody id="prod-body-${key}">
                <tr><td colspan="${activeSubTab === 'fuel' ? 7 : 8}" style="text-align:center;padding:14px;color:#64748b;"><i class="fas fa-spinner fa-spin"></i> Loading...</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; background:#f0f9ff; border:1px solid #bae6fd; border-radius:8px; padding:12px 16px; margin-top:14px;">
            <div style="display:flex; flex-direction:column; gap:2px;">
              <span style="font-size:10px; font-weight:700; color:#0369a1; text-transform:uppercase;">${activeSubTab === 'fuel' ? 'Total Fuel Types' : 'Total Products'}</span>
              <span id="total-count-${key}" style="font-size:16px; font-weight:800; color:#1e293b;">&mdash;</span>
            </div>
            ${activeSubTab === 'fuel' ? `<div style="display:flex; flex-direction:column; gap:2px; text-align:center;">
              <span style="font-size:10px; font-weight:700; color:#0369a1; text-transform:uppercase;">Total Liters Ordered</span>
              <span id="total-liters-${key}" style="font-size:16px; font-weight:800; color:#1e293b;">&mdash;</span>
            </div>` : ''}
...
function fetchPOItemsInline(key, r, isPending) {
  ...
  Promise.all(urls.map(url => fetch(url).then(res => res.json()).catch(() => ({success:false,items:[]})))) 
    .then(results => {
      ...
      const colspan = isFuel ? 7 : 8;
      if (allItems.length === 0) {
        targetTbody.innerHTML = `<tr><td colspan="${colspan}" style="text-align:center;padding:15px;color:#64748b;">No items found for this order.</td></tr>`;
        return;
      }
      let html = ''; let total = 0; let totalLiters = 0;
      allItems.forEach((item, idx) => {
        const rowBg = idx % 2 === 0 ? '#ffffff' : '#f8fafc';
        const qty   = parseFloat(item.quantity) || 0;
        const price = isPending ? 0 : parseFloat(item.unit_price || item.cost_price || item.approved_price || 0);
        const rowTotal = qty * price;
        total += rowTotal;

        if (isFuel) {
          const fuelId   = item.fi_id ? 'F' + String(item.fi_id).padStart(3,'0') : ('F' + String(idx+1).padStart(3,'0'));
          const fuelType = item.product_name || item.fuel_type_name || '-';
          const ugtNo    = item.ugt_no || '-';
          const tankCap  = parseFloat(item.tank_capacity) || 0;
          totalLiters += qty;

          if (isPending) {
            html += `<tr style="background:${rowBg};">
              <td style="padding:9px 12px;font-weight:700;color:#002F6C;">${escHtml(fuelId)}</td>
              <td style="padding:9px 12px;font-weight:600;">${escHtml(fuelType)}</td>
              <td style="padding:9px 12px;font-family:monospace;font-size:11px;">${escHtml(ugtNo)}</td>
              <td style="padding:9px 12px;text-align:right;">${tankCap > 0 ? tankCap.toLocaleString() + ' L' : '&mdash;'}</td>
              <td style="padding:9px 12px;text-align:right;font-weight:700;">${qty.toLocaleString()} L</td>
              <td style="padding:9px 12px;text-align:right;">
                <input type="number" step="0.01" min="0.01" name="items[${item.id}][price]" value="" placeholder="0.00" required
                  class="po-cost-input" style="width:100px;padding:4px;border:1px solid #cbd5e1;border-radius:4px;text-align:right;"
                  oninput="recalcGrandTotalInline('${key}')" data-qty="${qty}" data-row-id="${item.id}">
            ...
            const actualPrice = parseFloat(item.unit_price || item.cost_price || item.approved_price || 0);
            html += `<tr style="background:${rowBg};">
              <td style="padding:9px 12px;font-weight:700;color:#002F6C;">${escHtml(fuelId)}</td>
              <td style="padding:9px 12px;font-weight:600;">${escHtml(fuelType)}</td>
              <td style="padding:9px 12px;font-family:monospace;font-size:11px;">${escHtml(ugtNo)}</td>
              <td style="padding:9px 12px;text-align:right;">${tankCap > 0 ? tankCap.toLocaleString() + ' L' : '&mdash;'}</td>
              <td style="padding:9px 12px;text-align:right;font-weight:700;">${qty.toLocaleString()} L</td>
              <td style="padding:9px 12px;text-align:right;">₱${actualPrice.toFixed(2)}</td>
              <td style="padding:9px 12px;text-align:right;font-weight:700;">₱${(qty * actualPrice).toFixed(2)}</td>
            </tr>`;
          }
        } else {
          ...
          if (isPending) {
            html += `<tr style="background:${rowBg};">
              ...
              <td style="padding:9px 12px;text-align:right;">
                <input type="number" step="0.01" min="0.01" name="items[${item.id}][price]" value="" placeholder="0.00" required
                  class="po-cost-input" style="width:90px;padding:4px;border:1px solid #cbd5e1;border-radius:4px;text-align:right;"
                  oninput="recalcGrandTotalInline('${key}')" data-qty="${qty}" data-row-id="${item.id}">
                <input type="hidden" name="items[${item.id}][qty]" value="${qty}">
              </td>
              <td style="padding:9px 12px;text-align:right;font-weight:700;" id="rt-${key}-${item.id}">₱${rowTotal.toFixed(2)}</td>
      ...
      if (tc) tc.textContent = isFuel ? allItems.length + ' Fuel Type(s)' : allItems.length + ' Product(s)';
      const tl = document.getElementById('total-liters-' + key);
      if (tl && isFuel) tl.textContent = totalLiters.toLocaleString() + ' L';
