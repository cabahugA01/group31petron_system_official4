...
if (!in_array($role, ['admin', 'superadmin'])) {
    $_SESSION['error'] = 'Access denied. Administrator access required.';
...
$station_params = [];
if ($filter_station > 0) {
    $station_condition = "station_id = ?";
    $station_params[] = $filter_station;
} else {
...
if (isset($_GET['ajax_action'])) {
    ...
    if ($action === 'get_history') {
        $pump_id = (int)$_GET['pump_id'];
        try {
            // Retrieve pump to get station_id, pump_number and fuel_type name
            $stmt = $pdo->prepare("
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 1. ADD PUMP
    if ($action === 'add_pump') {
        $target_station = ($role === 'superadmin') ? (int)$_POST['station_id'] : $station_id;
        ...
        if ($target_station <= 0) {
            $_SESSION['error'] = 'Please select a valid station.';
        } elseif (empty($pump_number)) {
            $_SESSION['error'] = 'Pump name/number is required.';
        } elseif ($fuel_type_id <= 0) {
            $_SESSION['error'] = 'Please select a fuel type.';
        } else {
            try {
                $pdo->beginTransaction();

                // Check for duplicate pump number at target station
                $chk = $pdo->prepare("SELECT id FROM fuel_pumps WHERE station_id = ? AND LOWER(pump_number) = LOWER(?)");
                $chk->execute([$target_station, $pump_number]);
                if ($chk->fetch()) {
                    throw new Exception("A pump with that name/number already exists at this station.");
                }

                $stmt = $pdo->prepare("
                    INSERT INTO fuel_pumps (station_id, pump_number, fuel_type_id, capacity, status, calibration_value, created_at)
                    VALUES (?, ?, ?, ?, ?, 0.00, NOW())
                ");
                $stmt->execute([$target_station, $pump_number, $fuel_type_id, $capacity, $status]);
                $new_pump_id = $pdo->lastInsertId();

                log_activity($pdo, $me['id'], 'Add Pump', "Added new pump {$pump_number} (ID: {$new_pump_id}) at Station ID {$target_station}");
                write_audit_log($pdo, 'Add Pump', "Created pump {$pump_number} at station {$target_station}", 'fuel_pumps', $new_pump_id, 'system', 'Success');

                $pdo->commit();
                $_SESSION['success'] = "Pump <strong>" . htmlspecialchars($pump_number) . "</strong> has been created successfully.";
            } catch (Exception $e) {
                if ($pdo->inTransaction()) $pdo->rollBack();
                $_SESSION['error'] = "Error adding pump: " . $e->getMessage();
            }
        }
        header("Location: admin_pump_master_oversight.php?" . http_build_query($_GET));
        exit;
    }
    ...
    elseif ($action === 'edit_pump') {
        ...
        if ($pump_id <= 0) {
            ...
            try {
                ...
                // Verify pump exists
                $chk_exist = $pdo->prepare("SELECT * FROM fuel_pumps WHERE id = ?");
                $chk_exist->execute([$pump_id]);
                $existing = $chk_exist->fetch(PDO::FETCH_ASSOC);
                if (!$existing) {
                    throw new Exception("Pump not found.");
                }

                // Check for duplicate pump name/number at target station
                $chk_dup = $pdo->prepare("SELECT id FROM fuel_pumps WHERE station_id = ? AND LOWER(pump_number) = LOWER(?) AND id != ?");
                $chk_dup->execute([$target_station, $pump_number, $pump_id]);
                if ($chk_dup->fetch()) {
                    throw new Exception("Another pump with that name/number already exists at this station.");
                }

                $stmt = $pdo->prepare("
                    UPDATE fuel_pumps 
                    SET station_id = ?, pump_number = ?, fuel_type_id = ?, capacity = ?, status = ?
                    WHERE id = ?
                ");
                $stmt->execute([$target_station, $pump_number, $fuel_type_id, $capacity, $status, $pump_id]);

                log_activity($pdo, $me['id'], 'Edit Pump', "Updated pump ID {$pump_id} details (Number: {$pump_number})");
                write_audit_log($pdo, 'Edit Pump', "Updated pump ID {$pump_id} details", 'fuel_pumps', $pump_id, 'system', 'Success');

                $pdo->commit();
                $_SESSION['success'] = "Pump <strong>" . htmlspecialchars($pump_number) . "</strong> updated successfully.";
            } catch (Exception $e) {
                if ($pdo->inTransaction()) $pdo->rollBack();
                $_SESSION['error'] = "Error updating pump: " . $e->getMessage();
            }
        }
        header("Location: admin_pump_master_oversight.php?" . http_build_query($_GET));
        exit;
    ...
    elseif (in_array($action, ['activate', 'deactivate'])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT * FROM fuel_pumps WHERE id = ?");
            $stmt->execute([$pump_id]);
            $pump = $stmt->fetch(PDO::FETCH_ASSOC);
            ...
            // Verify station access if not superadmin
            if ($role !== 'superadmin' && (int)$pump['station_id'] !== $station_id) {
                throw new Exception("Access denied to this station's pump.");
            }

            $up = $pdo->prepare("UPDATE fuel_pumps SET status = ? WHERE id = ?");
            $up->execute([$new_status, $pump_id]);

            log_activity($pdo, $me['id'], ucfirst($action) . ' Pump', "Set status of pump ID {$pump_id} ({$pump['pump_number']}) to {$new_status}");
            write_audit_log($pdo, ucfirst($action) . ' Pump', "Set status of pump ID {$pump_id} to {$new_status}", 'fuel_pumps', $pump_id, 'system', 'Success');

            $pdo->commit();
            $_SESSION['success'] = "Pump <strong>" . htmlspecialchars($pump['pump_number']) . "</strong> is now " . strtolower($new_status) . ".";
        } catch (Exception $e) {
            if ($pdo->inTransaction()) $pdo->rollBack();
            $_SESSION['error'] = "Error toggling pump status: " . $e->getMessage();
        }
        header("Location: admin_pump_master_oversight.php?" . http_build_query($_GET));
        exit;
    }
    ...
    elseif ($action === 'update_calibration') {
        ...
        if ($pump_id <= 0) {
            ...
            $_SESSION['error'] = 'Reason is required for calibration updates.';
        } else {
            try {
                $pdo->beginTransaction();
                ...
                // Verify station access if not superadmin
                $target_station = (int)$pump['station_id'];
                if ($role !== 'superadmin' && $target_station !== $station_id) {
                    throw new Exception("Access denied to this station's pump.");
                }
                ...
                $up = $pdo->prepare("
                    ...
                    WHERE id = ?
                ");
                $up->execute([$calibration_value, $me['id'], $reason, $pump_id]);
                ...
                ");
                $up_inv->execute([$calibration_value, $target_station, $pump['fuel_type_id']]);
                ...
                write_audit_log($pdo, 'Update Calibration', "Updated pump {$pump['pump_number']} calibration to {$calibration_value} L", 'fuel_pumps', $pump_id, 'system', 'Success');

                $pdo->commit();
                $_SESSION['success'] = "Calibration for Pump <strong>" . htmlspecialchars($pump['pump_number']) . "</strong> updated successfully.";
            } catch (Exception $e) {
                if ($pdo->inTransaction()) $pdo->rollBack();
...
if ($assigned_tank_ft !== '') {
    $where[] = "ft.name = ?";
    $params[] = $assigned_tank_ft;
}

// Pump Status Filter
if ($pump_status_filter !== '') {
...
try {
    $sql = "
        ...
        $where_sql
        ORDER BY s.name ASC, fp.pump_number ASC, fp.id ASC
    ";
...
try {
    // Total Pumps
    $sp = $pdo->prepare("SELECT COUNT(*) FROM fuel_pumps WHERE $station_condition");
    $sp->execute($station_params);
    $total_pumps = (int)$sp->fetchColumn();

    // Active
    $sa = $pdo->prepare("SELECT COUNT(*) FROM fuel_pumps WHERE $station_condition AND status = 'Active'");
    $sa->execute($station_params);
    $active_pumps = (int)$sa->fetchColumn();

    // Inactive
    $si = $pdo->prepare("SELECT COUNT(*) FROM fuel_pumps WHERE $station_condition AND status = 'Inactive'");
    $si->execute($station_params);
    $inactive_pumps = (int)$si->fetchColumn();

    // Pumps requiring calibration: calibration_value is NULL or 0.00 or calibration_updated_at is NULL
    $sc = $pdo->prepare("SELECT COUNT(*) FROM fuel_pumps WHERE $station_condition AND (calibration_value IS NULL OR calibration_value = 0.00 OR calibration_updated_at IS NULL)");
    $sc->execute($station_params);
    $req_calibration = (int)$sc->fetchColumn();

    // Monthly updates count in pump_calibration_history
    $sm = $pdo->prepare("
        SELECT COUNT(*) 
    ...
    if ($export === 'excel') {
        ...
        <head>
            ...
            <style>
                ...
                th { background-color: #002F70; color: #ffffff; font-weight: bold; }
            </style>
        </head>
        <body>
            <h2>Pump Master Oversight Report</h2>
    ...
    if ($export === 'pdf') {
        ...
        <body onload="window.print()">
            ...
            <table class="meta-table">
                <tr>
                    <td><strong>Station:</strong> <?= ($filter_station > 0 && isset($pumps[0]['station_name'])) ? htmlspecialchars($pumps[0]['station_name']) : 'All Stations' ?></td>
                    <td style="text-align: right;"><strong>Run Date:</strong> <?= date('M d, Y H:i A') ?></td>
                </tr>
                <tr>
...
.pmo-fg input:focus, .pmo-fg select:focus {
...
<div class="pmo-cards">
    <!-- Total Pumps Card -->
    <div class="pmo-card blue">
        <div class="pmo-card-info">
            <span class="pmo-card-lbl">Total Pumps</span>
            <span class="pmo-card-val"><?= number_format($total_pumps) ?></span>
        </div>
        <div class="pmo-card-icon"><i class="fas fa-gas-pump"></i></div>
    </div>
    
    <!-- Active Pumps Card -->
    <div class="pmo-card green">
        <div class="pmo-card-info">
            <span class="pmo-card-lbl">Active Pumps</span>
            <span class="pmo-card-val"><?= number_format($active_pumps) ?></span>
        </div>
        <div class="pmo-card-icon"><i class="fas fa-toggle-on"></i></div>
    </div>
    
    <!-- Inactive Pumps Card -->
    <div class="pmo-card red">
        <div class="pmo-card-info">
            <span class="pmo-card-lbl">Inactive Pumps</span>
            <span class="pmo-card-val"><?= number_format($inactive_pumps) ?></span>
        </div>
        <div class="pmo-card-icon"><i class="fas fa-toggle-off"></i></div>
    </div>
    
    <!-- Pumps Requiring Calibration Card -->
    <div class="pmo-card yellow">
        <div class="pmo-card-info">
            <span class="pmo-card-lbl">Pumps Requiring Calibration</span>
            <span class="pmo-card-val"><?= number_format($req_calibration) ?></span>
        </div>
        <div class="pmo-card-icon"><i class="fas fa-balance-scale"></i></div>
    </div>
    
    <!-- Calibration Updates This Month Card -->
    <div class="pmo-card purple">
        <div class="pmo-card-info">
            <span class="pmo-card-lbl">Calibration Updates This Month</span>
            <span class="pmo-card-val"><?= number_format($cal_updates_month) ?></span>
        </div>
        <div class="pmo-card-icon"><i class="fas fa-history"></i></div>
    </div>
</div>

<!-- == FILTERS == -->
<form method="get" class="pmo-filter">
    <!-- Station Dropdown (SuperAdmin only) -->
    ...
    <div class="pmo-fg">
        ...
        <select name="fuel_type" onchange="this.form.submit()">
            ...
            <?php foreach ($fuel_types as $ft): ?>
                <option value="<?= htmlspecialchars($ft['name']) ?>" <?= $fuel_type_filter === $ft['name'] ? 'selected' : '' ?>>
    ...
    <div class="pmo-fg">
        ...
        <select name="pump_status" onchange="this.form.submit()">
    ...
    <!-- Assigned Tank Dropdown -->
    <div class="pmo-fg">
        <label>Assigned Tank</label>
        <select name="assigned_tank" onchange="this.form.submit()">
            <option value="">All Tanks</option>
            <?php foreach ($fuel_types as $ft): ?>
                <option value="<?= htmlspecialchars($ft['name']) ?>" <?= $assigned_tank_ft === $ft['name'] ? 'selected' : '' ?>>
                    <?= htmlspecialchars($ft['name']) ?> Tank
                </option>
            <?php endforeach; ?>
        </select>
    </div>
...
<div class="tbl-card">
    <div class="tbl-hd">
        <span class="tbl-title"><i class="fas fa-list"></i> Fuel Pump Inventory</span>
...
<!-- == MODAL: ADD PUMP == -->
<div id="addPumpModal" class="modal-overlay">
    <div class="modal-box">
        <form method="post">
            <input type="hidden" name="action" value="add_pump">
            <div class="modal-header">
                <h3>Add New Fuel Pump</h3>
                <span class="close" onclick="closeModal('addPumpModal')">&times;</span>
            </div>
            <div class="modal-body">
                ...
                <div class="modal-form-row">
                    ...
                    <input type="text" name="pump_number" required placeholder="e.g. Pump 1">
                </div>
                ...
                <div class="modal-form-row">
                    ...
                    <input type="number" step="0.01" name="capacity" value="0.00" required>
                </div>
            ...
            <div class="modal-footer">
                <button type="button" class="modal-btn modal-btn-secondary" onclick="closeModal('addPumpModal')">Cancel</button>
                <button type="submit" class="modal-btn modal-btn-primary">Save Pump</button>
            </div>
        </form>
    </div>
</div>
...
<!-- == MODAL: EDIT PUMP == -->
<div id="editPumpModal" class="modal-overlay">
    <div class="modal-box">
        <form method="post">
            <input type="hidden" name="action" value="edit_pump">
            <input type="hidden" name="pump_id" id="edit_pump_id">
            <div class="modal-header">
                <h3>Edit Fuel Pump Details</h3>
                <span class="close" onclick="closeModal('editPumpModal')">&times;</span>
            </div>
            <div class="modal-body">
                <?php if ($role === 'superadmin'): ?>
                    <div class="modal-form-row">
                ...
                <div class="modal-form-row">
                    <label>Pump Number / Name *</label>
                    <input type="text" name="pump_number" id="edit_pump_number" required>
                </div>
                
                <div class="modal-form-row">
                    <label>Fuel Type *</label>
                    <select name="fuel_type_id" id="edit_fuel_type_id" required>
                        <option value="">Select Fuel Type</option>
                        <?php foreach ($fuel_types as $ft): ?>
                            <option value="<?= $ft['id'] ?>"><?= htmlspecialchars($ft['name']) ?></option>
                        <?php endforeach; ?>
                ...
                <div class="modal-form-row">
                    <label>Capacity (Liters)</label>
                    <input type="number" step="0.01" name="capacity" id="edit_capacity" required>
                </div>
            ...
            <div class="modal-footer">
                <button type="button" class="modal-btn modal-btn-secondary" onclick="closeModal('editPumpModal')">Cancel</button>
        ...
        <form method="post">
            ...
            <div class="modal-body">
                ...
                <div class="modal-form-row">
                    ...
                    <textarea name="reason" required placeholder="Describe the reason (e.g. periodic validation, nozzle correction...)"></textarea>
                </div>
            </div>
            <div class="modal-footer">
    ...
    <div class="modal-box" style="max-width: 650px;">
        ...
        <div class="modal-body">
            <div style="overflow-x: auto;">
                <table class="pmo-tbl" style="font-size: 11px;">
                    <thead>
                        <tr>
                            <th>Update Date</th>
...
function openAddPumpModal() {
    document.getElementById('addPumpModal').classList.add('active');
}

function openEditPumpModal(p) {
    document.getElementById('edit_pump_id').value = p.id;
    document.getElementById('edit_pump_number').value = p.pump_number;
    document.getElementById('edit_fuel_type_id').value = p.fuel_type_id;
    document.getElementById('edit_capacity').value = p.capacity;
    document.getElementById('edit_status').value = p.status;
    
    var stationSel = document.getElementById('edit_station_id');
    if (stationSel) {
        stationSel.value = p.station_id;
    }
    
    document.getElementById('editPumpModal').classList.add('active');
}

function openCalibrationModal(id, name, fuelType, prevVal) {
...
function viewHistory(pumpId, pumpName) {
    ...
    fetch('admin_pump_master_oversight.php?ajax_action=get_history&pump_id=' + pumpId)
        .then(response => response.json())
        .then(data => {
            if (data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:20px;color:#94a3b8;"><i class="fas fa-history"></i> No calibration history found for this pump.</td></tr>';
            } else {
                var html = '';
                data.forEach(l => {
                    var prev = parseFloat(l.previous_calibration) || 0;
                    var next = parseFloat(l.new_calibration) || 0;
                    var diff = next - prev;
                    var diffStr = (diff >= 0 ? '+' : '') + diff.toFixed(3) + ' L';
                    var diffColor = diff > 0 ? '#16a34a' : (diff < 0 ? '#dc2626' : '#475569');
                    var dateVal = new Date(l.updated_at);
                    var dateStr = dateVal.toLocaleDateString(undefined, {month:'short', day:'numeric', year:'numeric'})
                                + ' ' + dateVal.toLocaleTimeString(undefined, {hour:'2-digit', minute:'2-digit'});
        ...
        .catch(err => {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:20px;color:#dc2626;"><i class="fas fa-exclamation-circle"></i> Error loading history.</td></tr>';
        });
}
</script>
