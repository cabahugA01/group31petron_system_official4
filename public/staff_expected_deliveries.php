<?php
$page_id = 'staff_expected_deliveries';
...
/* ── Summary Cards Data ── */
$total_expected = count($expected_deliveries);
$pending_this_week = 0;
$overdue = 0;

foreach ($expected_deliveries as $ed) {
    $delivery_date = strtotime($ed['created_at']);
    $week_start = strtotime('monday this week');
    $week_end = strtotime('sunday this week');
    
    if ($delivery_date >= $week_start && $delivery_date <= $week_end) {
        $pending_this_week++;
    }
    
    // Check if overdue (created more than 7 days ago)
    if ($delivery_date < strtotime('-7 days')) {
        $overdue++;
    }
}
...
.btn-receive { background: #28a745; color: #fff; border: none; padding: 10px 18px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 6px; transition: background .2s; }
...
<div class="page-head">
    <div>
        <h1 class="h1"><i class="fas fa-clipboard-list"></i> Expected Deliveries</h1>
        <div class="sub">View POs created by Manager/Admin with expected items and delivery dates.</div>
    </div>
...
<div class="summary-grid">
    <div class="summary-card sc-blue" style="position:relative;">
        <i class="fas fa-boxes sc-icon"></i>
        <div class="sc-num"><?php echo $total_expected; ?></div>
        <div class="sc-label">Total Expected</div>
    </div>
    <div class="summary-card sc-orange" style="position:relative;">
        <i class="fas fa-calendar-week sc-icon"></i>
        <div class="sc-num"><?php echo $pending_this_week; ?></div>
        <div class="sc-label">Pending This Week</div>
    </div>
    <div class="summary-card sc-red" style="position:relative;">
        <i class="fas fa-exclamation-triangle sc-icon"></i>
        <div class="sc-num"><?php echo $overdue; ?></div>
...
<div class="del-card">
    <div class="del-card-head">
        <div class="del-card-title">
            <i class="fas fa-truck-loading"></i> Expected Merchandise Deliveries
            <?php if($total_expected > 0): ?>
                <span style="background:#dc3545;color:#fff;border-radius:12px;padding:3px 10px;font-size:12px;"><?php echo $total_expected; ?></span>
            <?php endif; ?>
        </div>
        <span style="font-size:13px;color:#6c757d;">From Finalized Purchase Orders</span>
    </div>
    <div class="del-card-body">
        <?php if (empty($expected_deliveries)): ?>
            <div class="empty-state">
                <i class="fas fa-box-open"></i>
                <p>No expected merchandise deliveries at the moment.</p>
                <p style="font-size:13px;color:#6c757d;margin-top:10px;">Check back later or use Manual Encode for non-PO deliveries.</p>
                <a href="staff_record_delivery.php" style="display:inline-block;margin-top:20px;background:#002F70;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;">
            ...
            <div class="expected-item">
                <div class="expected-info">
                    ...
                    <div class="expected-meta">
                        ...
                        <span><i class="fas fa-box"></i> Expected Qty: <strong><?php echo number_format($ed['quantity'], 2) . ' ' . $ed['unit']; ?></strong></span>
                        <span><i class="fas fa-building"></i> <?php echo htmlspecialchars($ed['supplier']); ?></span>
                        <span><i class="fas fa-calendar"></i> <?php echo date('M d, Y', strtotime($ed['created_at'])); ?></span>
                    </div>
                </div>
