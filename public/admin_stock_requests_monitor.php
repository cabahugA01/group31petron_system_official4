...
try {
    ...
    $stmt1 = $pdo->prepare("
        SELECT sr.*, COALESCE(u.name, 'Unknown Staff') AS staff_name, m.name AS manager_name, po.po_number, po.status AS po_status
...
<div class="page-head">
  <div>
    <h1 class="h1"><i class="fas fa-eye"></i> Stock Request Monitoring</h1>
    <div class="sub">Monitor staff requests, manager validation, and generated purchase orders. Read-only control dashboard.</div>
  </div>
...
<div class="kpi-grid">
  <div class="kpi-card"><div class="kv" style="color:var(--blue)"><?= count($merch_reqs)+count($fuel_reqs) ?></div><div class="kl">Total Requests</div></div>
  <div class="kpi-card"><div class="kv" style="color:var(--orange)"><?= $tot_pending ?></div><div class="kl">Pending Processing</div></div>
  <div class="kpi-card"><div class="kv" style="color:var(--green)"><?= count(array_filter($merch_reqs, fn($r)=>in_array($r['status'],['Approved','Forwarded to Admin']))) + count(array_filter($fuel_reqs, fn($r)=>$r['status']==='Approved')) ?></div><div class="kl">Validated / Approved</div></div>
</div>

<div class="main-tab-nav">
  <button class="tab-btn active" id="tabMerchBtn" onclick="switchTab('merch')"><i class="fas fa-boxes"></i> Merchandise Requests (<?= count($merch_reqs) ?>)</button>
  <button class="tab-btn" id="tabFuelBtn" onclick="switchTab('fuel')"><i class="fas fa-gas-pump"></i> Fuel Requests (<?= count($fuel_reqs) ?>)</button>
</div>

<!-- Merchandise Panel -->
<div class="card" id="panelMerch">
  <div class="card-hd">
    <div class="card-hd-title"><i class="fas fa-boxes"></i> Merchandise Request Pipeline <span id="vcntMerch" style="font-size:12px;color:var(--gray);font-weight:400;margin-left:6px;"></span></div>
  </div>
  <div class="card-body">
    <div class="toolbar">
      <input type="text" id="sqMerch" placeholder="&#128269; Search product, SKU..." oninput="filterMerch()">
    ...
    <div class="table-wrap">
      <table class="reqs">
        <colgroup>
          <col style="width:7%"><col style="width:13%"><col style="width:13%"><col style="width:20%">
          <col style="width:9%"><col style="width:9%"><col style="width:13%"><col style="width:16%">
        </colgroup>
        <thead>
          <tr>
            <th>ID</th><th>Date</th><th>Staff</th><th>Product</th><th>Req Qty</th><th>App Qty</th><th>Status</th><th>PO/Notes</th>
          </tr>
        </thead>
        <tbody id="merchB">
        <?php if (empty($merch_reqs)): ?>
          <tr><td colspan="8" style="text-align:center;padding:32px;color:var(--gray);">No merchandise requests found.</td></tr>
        <?php else: foreach ($merch_reqs as $r):
          $st = strtolower($r['status']);
          $bc = $st==='pending'?'badge-pending':($st==='forwarded to admin'?'badge-forwarded':($st==='approved'?'badge-approved':'badge-rejected'));
        ?>
          <tr class="merch-row" data-p="<?= strtolower(htmlspecialchars($r['item_name'])) ?>" data-s="<?= strtolower(htmlspecialchars($r['item_sku']??'')) ?>" data-x="<?= $st ?>">
            ...
            <td><?= $r['approved_quantity']!==null ? (int)$r['approved_quantity'] : '—' ?></td>
            <td><span class="badge <?= $bc ?>"><?= htmlspecialchars($r['status']) ?></span></td>
            <td>
              <?php if ($r['po_number']): ?>
                <strong style="color:var(--blue);"><?= htmlspecialchars($r['po_number']) ?></strong>
              <?php elseif ($r['manager_notes']): ?>
                <span style="color:var(--gray);font-style:italic;" title="<?= htmlspecialchars($r['manager_notes']) ?>"><?= htmlspecialchars($r['manager_notes']) ?></span>
              <?php else: ?>
...
<div class="card" id="panelFuel" style="display:none;">
  <div class="card-hd">
    <div class="card-hd-title"><i class="fas fa-gas-pump"></i> Fuel Request Pipeline <span id="vcntFuel" style="font-size:12px;color:var(--gray);font-weight:400;margin-left:6px;"></span></div>
  </div>
  <div class="card-body">
    <div class="toolbar">
      <input type="text" id="sqFuel" placeholder="&#128269; Search fuel type..." oninput="filterFuel()">
    ...
    <div class="table-wrap">
      <table class="reqs">
        ...
        <thead>
          <tr>
            <th>ID</th><th>Date</th><th>Staff</th><th>Fuel Type</th><th>Req Liters</th><th>App Liters</th><th>Status</th><th>Notes</th>
          </tr>
        </thead>
        <tbody id="fuelB">
        <?php if (empty($fuel_reqs)): ?>
          <tr><td colspan="8" style="text-align:center;padding:32px;color:var(--gray);">No fuel requests found.</td></tr>
        <?php else: foreach ($fuel_reqs as $r):
          $st = strtolower($r['status']);
          $bc = $st==='pending'?'badge-pending':($st==='approved'?'badge-approved':'badge-rejected');
        ?>
          <tr class="fuel-row" data-f="<?= strtolower(htmlspecialchars($r['fuel_type'])) ?>" data-x="<?= $st ?>">
            ...
            <td><?= $r['approved_liters']!==null ? number_format((float)$r['approved_liters'],2) : '—' ?></td>
            <td><span class="badge <?= $bc ?>"><?= htmlspecialchars($r['status']) ?></span></td>
            <td>
              <span style="color:var(--gray);font-style:italic;" title="<?= htmlspecialchars($r['manager_notes'] ?: $r['remarks'] ?: '') ?>">
                <?= htmlspecialchars($r['manager_notes'] ?: $r['remarks'] ?: '—') ?>
              </span>
            </td>
          </tr>
        <?php endforeach; endif; ?>
...
function filterMerch() {
    ...
    var s = document.getElementById('sfMerch').value;
    var rows = document.querySelectorAll('#merchB .merch-row'), v=0;
    rows.forEach(function(r) {
        var show = (!q || r.dataset.p.includes(q) || r.dataset.s.includes(q)) && (!s || r.dataset.x === s);
...
function filterFuel() {
    ...
    var s = document.getElementById('sfFuel').value;
    var rows = document.querySelectorAll('#fuelB .fuel-row'), v=0;
    rows.forEach(function(r) {
        var show = (!q || r.dataset.f.includes(q)) && (!s || r.dataset.x === s);
        r.style.display = show ? '' : 'none'; if(show) v++;
    });
