...
try {
    $s = $pdo->prepare("SELECT
        COUNT(*) as total,
        SUM(CASE WHEN LOWER(fvr.status)='open' THEN 1 ELSE 0 END) as open_c,
        SUM(CASE WHEN LOWER(fvr.status)='under investigation' THEN 1 ELSE 0 END) as invest_c,
        SUM(CASE WHEN LOWER(fvr.status)='resolved' THEN 1 ELSE 0 END) as resolved_c,
        SUM(ABS(fvr.variance_liters)) as total_var
        FROM fuel_variance_reports fvr WHERE $where_sql");
    $s->execute($params);
    $row = $s->fetch(PDO::FETCH_ASSOC);
    $cnt_total    = (int)($row['total']     ?? 0);
    $cnt_open     = (int)($row['open_c']    ?? 0);
    $cnt_invest   = (int)($row['invest_c']  ?? 0);
    $cnt_resolved = (int)($row['resolved_c']?? 0);
...
try {
    $stmt = $pdo->prepare("SELECT fvr.*,
        ...
        LEFT JOIN users rb    ON fvr.resolved_by = rb.id
        WHERE $where_sql
        ORDER BY FIELD(LOWER(fvr.status),'open','under investigation','resolved'), fvr.report_date DESC
        LIMIT 500");
    $stmt->execute($params);
...
<div class="afao-cards">
    <div class="afao-card c-blue">
        ...
        <div class="afao-card-meta">
            ...
            <span>All variance entries</span>
        </div>
    </div>
    <div class="afao-card c-red">
        <div class="afao-card-ico"><i class="fas fa-exclamation-circle"></i></div>
        <div class="afao-card-meta">
            <h3>Open</h3>
            <h2><?= number_format($cnt_open) ?></h2>
            <span>Needs attention</span>
        </div>
    </div>
    <div class="afao-card c-amber">
        <div class="afao-card-ico"><i class="fas fa-search"></i></div>
        <div class="afao-card-meta">
            <h3>Investigating</h3>
            <h2><?= number_format($cnt_invest) ?></h2>
            <span>Under review</span>
        </div>
    </div>
    <div class="afao-card c-green">
        <div class="afao-card-ico"><i class="fas fa-check-circle"></i></div>
        <div class="afao-card-meta">
            <h3>Resolved</h3>
            <h2><?= number_format($cnt_resolved) ?></h2>
            <span>Total variance: <?= number_format($sum_variance,2) ?> L</span>
        </div>
    </div>
</div>
...
<form method="get" class="afao-filter">
    ...
    <div class="afao-fg">
        ...
        <select name="status">
            ...
            <option value="under investigation" <?= $filter_status==='under investigation'?'selected':'' ?>>Investigating</option>
...
<div class="afao-table-card">
    ...
    <div class="afao-empty">
        ...
        <div style="font-size:13px;">No variance/reconciliation entries for the selected period and filters.</div>
    </div>
    <?php else: ?>
    <div style="overflow-x:hidden;max-width:100%;">
        <table class="afao-tbl" id="reconTable">
            <colgroup>
                <col style="width:7%"><col style="width:9%"><col style="width:11%"><col style="width:9%">
            ...
            <tbody>
                <?php foreach ($records as $r):
                    ...
                    $var_pct = abs($r['variance_percent'] ?? 0);
                    $lc = ($r['variance_liters'] ?? 0) < 0 ? 'color:#dc2626;' : 'color:#16a34a;';
                    if ($sl === 'open')                  $st_color = 'color:#dc2626;font-weight:700;';
                    elseif ($sl === 'under investigation') $st_color = 'color:#d97706;font-weight:700;';
                    else                                   $st_color = 'color:#16a34a;font-weight:700;';
                ?>
                <tr>
                    ...
                    <td style="font-weight:700;font-family:monospace;<?= $lc ?>">
                        <?= ($r['variance_liters'] > 0 ? '+' : '') . number_format($r['variance_liters'],2) ?> L
                    </td>
