...
/* View dropdown */
.cal-view-dropdown { position: absolute; top: 100%; right: 0; margin-top: 4px; background: #fff; border: 1px solid #dadce0; border-radius: 8px; box-shadow: 0 2px 6px rgba(0,0,0,.2); min-width: 160px; z-index: 100; display: none; }
    ...
    <div class="cal-sidebar">
        ...
        <div class="cal-mini-month">
            <div class="cal-mini-header">
                <span class="cal-mini-title"><?= date('F Y') ?></span>
                <div>
                    <button class="cal-mini-nav" onclick="navigateMiniMonth(-1)"><i class="fas fa-chevron-left"></i></button>
                    <button class="cal-mini-nav" onclick="navigateMiniMonth(1)"><i class="fas fa-chevron-right"></i></button>
                </div>
            </div>
            <div class="cal-mini-grid">
                <?php foreach(['S','M','T','W','T','F','S'] as $d): ?>
                <div class="cal-mini-weekday"><?= $d ?></div>
                <?php endforeach; ?>
                <?php
                $mini_start = new DateTime('first day of this month');
                $mini_start_dow = (int)$mini_start->format('w');
                for ($i = 0; $i < $mini_start_dow; $i++) {
                    echo '<div class="cal-mini-day"></div>';
                }
                $mini_end = new DateTime('last day of this month');
                $days_in_month = (int)$mini_end->format('d');
                for ($d = 1; $d <= $days_in_month; $d++):
                    $is_today = ($d == date('j') && date('Y-m') == $current_month_str);
                ?>
                <div class="cal-mini-day <?= $is_today ? 'today' : '' ?>"><?= $d ?></div>
                <?php endfor; ?>
            </div>
        </div>
