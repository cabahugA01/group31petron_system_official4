...
try {
    echo "Checking table job_order_service_types...\n";
    $stmt = $pdo->query("SHOW TABLES LIKE 'job_order_service_types'");
    if ($stmt->rowCount() == 0) {
        echo "Table does not exist. Creating table...\n";
    } else {
        echo "Table exists.\n";
        $cols = $pdo->query("SHOW COLUMNS FROM job_order_service_types")->fetchAll(PDO::FETCH_ASSOC);
        print_r($cols);
    }
