...
foreach ($vts as $v) {
    $tid = $v['transaction_id'];
    $s3 = $pdo->prepare("SELECT id FROM merchandise_transactions WHERE transaction_id = ?");
    $s3->execute([$tid]);
    $found = $s3->fetch();
    echo "Voided TXN $tid exists in merchandise_transactions: " . ($found ? "YES (id={$found['id']})" : "NO") . "\n";
    
    if ($found) {
        $s4 = $pdo->prepare("SELECT product_name, quantity, unit_price, item_type FROM merchandise_transaction_items WHERE transaction_id = ?");
        $s4->execute([$found['id']]);
        $items = $s4->fetchAll(PDO::FETCH_ASSOC);
