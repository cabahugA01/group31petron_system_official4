...
// Manager metrics - use existing database tables
$metrics = [
    'staff_on_duty' => 0,
    'pending_approvals' => 0,
    'active_jobs' => 0,
...
try {
    ...
    // Pending approvals - use sales table with status filtering
    $stmt = $pdo->prepare("SELECT COUNT(*) FROM sales WHERE station_id = ? AND status = 'Pending Validation'");
    $stmt->execute([$station_id]);
    ...
    // Quality score (placeholder - calculate from recent performance)
    $metrics['quality_score'] = 95.0;
    
    // Shift coverage (placeholder - calculate from shift logs)
    $metrics['shift_coverage'] = 85.0;
    
    // Compliance checks (placeholder - count passed checks)
    $metrics['compliance_checks'] = 8;
...
<div class="manager-stats-grid">
  ...
  <div class="stat-card">
    ...
    <div class="stat-card">
        <div class="stat-icon bg-warning"><i class="fas fa-clipboard-check"></i></div>
        <div class="stat-content">
            <div class="stat-value"><?php echo $metrics['pending_approvals']; ?></div>
            <div class="stat-label">Pending Approvals</div>
            <div class="stat-sub">Awaiting action</div>
        </div>
    </div>
    <div class="stat-card">
        ...
        <div class="stat-content">
            <div class="stat-value"><?php echo $metrics['active_jobs']; ?></div>
            <div class="stat-label">Active Jobs</div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-content">
            <div class="stat-value"><?php echo number_format($metrics['quality_score'], 1); ?>%</div>
            <div class="stat-label">Quality Score</div>
            <div class="stat-sub">Performance rating</div>
        </div>
    </div>
    <div class="stat-card">
        <div class="stat-icon bg-info"><i class="fas fa-calendar"></i></div>
        <div class="stat-content">
            <div class="stat-value"><?php echo number_format($metrics['shift_coverage'], 1); ?>%</div>
    ...
    <div class="stat-card">
        ...
        <div class="stat-content">
            <div class="stat-value"><?php echo $metrics['compliance_checks']; ?></div>
            <div class="stat-label">Compliance Checks</div>
            <div class="stat-sub">Passed/Failed</div>
        </div>
    </div>
</div>
...
<div class="dashboard-grid">
    <div class="dashboard-column">
        <div class="dashboard-card">
            <div class="card-header">
                <h3><i class="fas fa-chart-line text-warning"></i> Quick Actions</h3>
            </div>
            <div class="card-body">
                <div class="quick-actions-grid">
                    <a href="transactions.php" class="action-btn">
                        <i class="fas fa-eye"></i>
                        <span>Transactions Oversight</span>
                    </a>
                    <a href="staff_management.php" class="action-btn">
                        <i class="fas fa-users"></i>
                        <span>Staff Management</span>
                    </a>
                    <a href="inventory_management.php" class="action-btn">
                        <i class="fas fa-box"></i>
                        <span>Inventory</span>
                    </a>
                    <a href="fuel_management.php" class="action-btn">
                        <i class="fas fa-gas-pump"></i>
                        <span>Fuel Operations</span>
                    </a>
                </div>
            </div>
        </div>
    ...
    <div class="dashboard-column">
        <div class="dashboard-card">
            <div class="card-header">
                <h3><i class="fas fa-users text-success"></i> Staff Status</h3>
            </div>
            <div class="card-body">
                <div class="staff-status-grid">
                    <div class="staff-status-item">
                        <h4>On Duty</h4>
                        <div class="status-number"><?php echo $metrics['staff_on_duty']; ?></div>
                        <div class="status-label">Currently active</div>
                    </div>
                    <div class="staff-status-item">
                        <h4>Off Duty</h4>
                        <div class="status-number"><?php echo ($metrics['staff_on_duty'] > 0 ? ($metrics['staff_on_duty'] - 1) : 0); ?></div>
                        <div class="status-label">Currently off duty</div>
                    </div>
                </div>
            </div>
        </div>
