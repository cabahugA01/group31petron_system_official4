...
$stmt = $pdo->prepare("
    SELECT id, item_name, item_id, requested_quantity, approved_quantity, status, created_at, manager_id
    FROM stock_requests 
    WHERE station_id = ?
    ORDER BY created_at DESC
");
$stmt->execute([$station_id]);
$manager_requests = $stmt->fetchAll(PDO::FETCH_ASSOC);

echo "Total Merchandise Requests: " . count($manager_requests) . "\n\n";

$pending = 0; $approved = 0; $rejected = 0;
foreach ($manager_requests as $req) {
...
$stmt = $pdo->prepare("
    SELECT po.id, po.request_id, po.product_name, po.quantity, po.status, po.created_at,
           sr.id as source_request_id, sr.item_name as source_item_name
...
");
$stmt->execute([$station_id]);
$admin_pos = $stmt->fetchAll(PDO::FETCH_ASSOC);

echo "Total Merchandise POs: " . count($admin_pos) . "\n\n";

if (count($admin_pos) > 0) {
    echo "PO Details:\n";
    foreach ($admin_pos as $po) {
        echo "  PO ID: {$po['id']} | Request ID: {$po['request_id']} | {$po['product_name']} | Qty: {$po['quantity']} | Status: {$po['status']}\n";
    }
}
...
$stmt = $pdo->prepare("
    SELECT sr.id, sr.item_name, sr.requested_quantity, sr.status, sr.created_at
    FROM stock_requests sr
    LEFT JOIN purchase_orders po ON po.request_id = sr.id AND po.type = 'merch'
    WHERE sr.station_id = ?
      AND LOWER(sr.status) LIKE '%approved%'
      AND po.id IS NULL
...
if (count($orphaned_requests) > 0) {
    echo "⚠ Found " . count($orphaned_requests) . " approved requests WITHOUT purchase orders:\n\n";
    foreach ($orphaned_requests as $req) {
        echo "  Request ID: {$req['id']} | {$req['item_name']} | Qty: {$req['requested_quantity']} | Status: {$req['status']} | Date: {$req['created_at']}\n";
    }
    echo "\n";
    echo "❌ ISSUE: Manager approved these requests but POs were NOT created!\n";
} else {
    echo "✅ No orphaned requests. All approved requests have corresponding POs.\n";
}
...
$stmt = $pdo->prepare("
    SELECT id, fuel_type, requested_liters, approved_liters, status, created_at
...
echo "Total Fuel Requests: " . count($fuel_requests) . "\n\n";

$fuel_pending = 0; $fuel_approved = 0; $fuel_rejected = 0;
foreach ($fuel_requests as $req) {
    $st = strtolower($req['status']);
    if (str_contains($st, 'pending')) $fuel_pending++;
    elseif (str_contains($st, 'approved')) $fuel_approved++;
    elseif (str_contains($st, 'rejected')) $fuel_rejected++;
}
...
$stmt = $pdo->prepare("
    SELECT fpo.id, fpo.po_number, ft.name AS fuel_name, fpo.volume, fpo.status, fpo.created_at
...
if (count($fuel_pos) > 0) {
    echo "Fuel PO Details:\n";
    foreach ($fuel_pos as $po) {
        echo "  PO ID: {$po['id']} | PO#: {$po['po_number']} | {$po['fuel_name']} | Volume: {$po['volume']} L | Status: {$po['status']}\n";
    }
}
...
if (count($manager_requests) == count($admin_pos) && count($orphaned_requests) == 0) {
    echo "✅ PERFECT SYNC! Manager and Admin have matching data.\n";
} else {
    echo "⚠ SYNC ISSUE DETECTED!\n";
    echo "\nPOSSIBLE CAUSES:\n";
