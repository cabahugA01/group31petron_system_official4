...
foreach ($merch_pos as $po) {
    echo "  ID: {$po['id']} | PO: {$po['po_number']} | {$po['product_name']} | Qty: {$po['quantity']} | Status: {$po['status']} | Date: {$po['created_at']}\n";
}
...
foreach ($fuel_pos as $po) {
    echo "  ID: {$po['id']} | PO: {$po['po_number']} | {$po['fuel_name']} | Volume: {$po['volume']} | Status: {$po['status']} | Date: {$po['created_at']}\n";
}
...
if (count($merch_pos) >= 12 && count($fuel_pos) >= 4) {
