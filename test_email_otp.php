<?php
/**
 * Test Email OTP Sending with Detailed Debugging
 * Run this script to verify OTP emails are working
 */
...
echo "<h2>Testing OTP Email Sending - Detailed Debug</h2>";
...
if (file_exists($phpmailer_path)) {
    echo "<p>✓ PHPMailer found at: {$phpmailer_path}</p>";
} else {
    die("<p style='color:red;'>❌ ERROR: PHPMailer not found at: {$phpmailer_path}</p>");
}

// Test if function exists
if (!function_exists('sendPasswordResetOTP')) {
    die("<p style='color:red;'>❌ ERROR: sendPasswordResetOTP function not found!</p>");
}
...
try {
    ...
    if ($result) {
        echo "<p style='color:green;font-weight:bold;font-size:18px;'>✅ SUCCESS! Email sent successfully.</p>";
        echo "<p>Check your inbox (and spam/junk folder) for the OTP email at: <strong>{$test_email}</strong></p>";
    } else {
        echo "<p style='color:red;font-weight:bold;font-size:18px;'>❌ FAILED! Could not send email.</p>";
        echo "<p><strong>Possible issues:</strong></p>";
        echo "<ul>";
        echo "<li>Gmail App Password might be incorrect</li>";
        echo "<li>Check if 2-Step Verification is enabled on Gmail account</li>";
        echo "<li>Internet connection issue</li>";
        echo "<li>Gmail might be blocking the connection</li>";
        echo "</ul>";
    }
} catch (Exception $e) {
...
echo "<p>Apache Error Log: <code>C:\\xampp\\apache\\logs\\error.log</code></p>";
