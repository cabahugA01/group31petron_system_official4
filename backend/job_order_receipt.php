<?php
/**
 * Job Order Receipt — Backend
 *
 * Dual-purpose file:
 *   1. Class JobOrderReceipt  — used by joborder.php after INSERT to save receipt to DB
 *   2. GET ?action=print&job_order_id=XXX — outputs a full printable HTML page
 */
...
class JobOrderReceipt {
    ...
    private function generateReceiptNumber() {
        ...
        try {
            $stmt = $this->pdo->prepare(
                "SELECT COUNT(*) AS cnt FROM job_order_receipts
    ...
    private function ensureTable() {
        try {
            $this->pdo->exec("
                CREATE TABLE IF NOT EXISTS job_order_receipts (
                    ...
                    INDEX idx_jo  (job_order_id),
                    INDEX idx_stn (station_id)
                )
            ");
        } catch (Exception $e) { /* already exists */ }
    }

    // ── Fetch job order row with joined names ─────────────────────────────────
    public function fetchJobOrder($job_order_id) {
        // Try by job_order_id first, then by job_order_number (same value)
        $stmt = $this->pdo->prepare("
            SELECT jo.*,
                   ...
                   COALESCE(m.full_name, 'Unassigned') AS mechanic_name,
                   COALESCE(u.name, 'Staff')        AS created_by_name
            FROM   job_orders jo
            ...
            LIMIT  1
        ");
        $stmt->execute([$job_order_id, $job_order_id]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    // ── Build receipt HTML fragment ───────────────────────────────────────────
    public function buildReceiptHtml($job) {
        ob_start();
        $job_order_data = $job;
        include __DIR__ . '/../templates/job_order_receipt.php';
        return ob_get_clean();
    }

    // ── Save receipt to DB ────────────────────────────────────────────────────
    public function saveReceipt($job_order_id, $html, $receipt_number, $job) {
        $this->ensureTable();
        try {
            $stmt = $this->pdo->prepare("
                INSERT INTO job_order_receipts
                    (job_order_id, receipt_number, receipt_html, receipt_data, station_id, created_by)
                VALUES (?, ?, ?, ?, ?, ?)
                ON DUPLICATE KEY UPDATE
                    receipt_html = VALUES(receipt_html),
                    receipt_data = VALUES(receipt_data),
                    updated_at   = NOW()
            ");
            $stmt->execute([
                $job_order_id,
            ...
            // Stamp receipt_number back onto the job order row
            try {
                $this->pdo->prepare(
                    "UPDATE job_orders SET receipt_number = ? WHERE job_order_id = ? OR job_order_number = ?"
                )->execute([$receipt_number, $job_order_id, $job_order_id]);
    ...
    // ── Main entry: generate + save ───────────────────────────────────────────
    public function generateAndSaveReceipt($job_order_id) {
        try {
            $job = $this->fetchJobOrder($job_order_id);
            if (!$job) {
                return ['success' => false, 'error' => "Job order not found: $job_order_id"];
            }

            $receipt_number = $this->generateReceiptNumber();
            $html           = $this->buildReceiptHtml($job);

            $this->saveReceipt($job_order_id, $html, $receipt_number, $job);
            ...
            try {
                $this->pdo->prepare("
                    ...
                    VALUES (?, 'RECEIPT_GENERATED', 'Created', 'Generated', ?, NOW(), ?, ?, ?)
                ")->execute([
                    $job['id'] ?? $job_order_id,
                    $_SESSION['user_id'] ?? null,
                    "Receipt $receipt_number generated for $job_order_id",
                    $_SERVER['REMOTE_ADDR']     ?? '',
                    $_SERVER['HTTP_USER_AGENT'] ?? '',
                ]);
            } catch (Exception $e) { /* audit table may differ */ }

            return ['success' => true, 'receipt_number' => $receipt_number, 'html' => $html];

        } catch (Exception $e) {
            error_log("generateAndSaveReceipt error: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action'])) {
    ...
    if ($_GET['action'] === 'print') {
        ...
        $job_order_id = trim($_GET['job_order_id'] ?? '');
        if ($job_order_id === '') {
            http_response_code(400);
            echo '<p style="font-family:sans-serif;color:red;padding:20px">Missing job_order_id parameter.</p>';
            exit;
        }

        $gen = new JobOrderReceipt($pdo, $station_id);
        $job = $gen->fetchJobOrder($job_order_id);

        if (!$job) {
            http_response_code(404);
            echo '<p style="font-family:sans-serif;color:red;padding:20px">Job order <strong>'
                 . htmlspecialchars($job_order_id) . '</strong> not found.</p>';
            exit;
        }

        // Build receipt fragment
        $receipt_html = $gen->buildReceiptHtml($job);

        // Determine receipt number (use saved one or generate on-the-fly)
        $receipt_number = $job['receipt_number'] ?? ('RCPT-' . strtoupper(substr(md5($job_order_id), 0, 8)));

        header('Content-Type: text/html; charset=UTF-8');
        $jo_safe = htmlspecialchars($job_order_id);
        ?>
<!DOCTYPE html>
<html lang="en">
<head>
...
.jo-r-address,.jo-r-tin{font-size:9.5px;color:#555;margin-top:1px}
...
/* Badges */
.jo-r-badge{display:inline-block;font-size:8px;font-weight:700;padding:1px 5px;
...
.s-pending-payment{background:#dc2626}.s-credit{background:#7c3aed}
...
window.onafterprint = function() {
    ...
    // Other actions (generate, history) — JSON responses
    $gen = new JobOrderReceipt($pdo, $station_id);

    switch ($_GET['action']) {
        case 'generate':
            $job_order_id = $_GET['job_order_id'] ?? '';
            echo json_encode($gen->generateAndSaveReceipt($job_order_id));
            break;
        default:
