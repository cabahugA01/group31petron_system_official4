<?php
/**
 * Job Orders Export Handler
 * Included by manager_reports_api.php for the 'export_job_orders' action.
...
$cnt_stmt = $pdo->prepare("
    SELECT status, COUNT(*) AS cnt, COALESCE(SUM(estimated_cost),0) AS total_cost
    FROM job_orders WHERE station_id=? AND DATE(created_at) BETWEEN ? AND ?
    GROUP BY status
");
$cnt_stmt->execute([$station_id, $date_from, $date_to]);
$counts = $cnt_stmt->fetchAll(PDO::FETCH_ASSOC);
...
$filename = 'job_orders_report_' . $date_from . '_to_' . $date_to;
...
if ($fmt === 'csv') {
    ...
    fputcsv($out, ['Status', 'Count', 'Total Cost (PHP)']);
    foreach ($counts as $c) {
        fputcsv($out, [$c['status'], $c['cnt'], number_format((float)$c['total_cost'], 2)]);
    }
    fputcsv($out, []);

    // Detail
    fputcsv($out, ['JOB ORDERS DETAIL']);
    fputcsv($out, ['#', 'Customer', 'Service Type', 'Description', 'Status', 'Cost (PHP)', 'Staff', 'Mechanic / Technician', 'Date & Time']);
    foreach ($rows as $r) {
        fputcsv($out, [
            $r['id'],
            $r['customer'],
    ...
    if ($rows) {
        fputcsv($out, ['', '', '', '', 'TOTAL', number_format(array_sum(array_column($rows, 'cost')), 2), '', '', '']);
    }
    fclose($out);
    exit;
}
...
$status_colors = [
    'pending' => '#92400e', 'pending validation' => '#92400e',
    'approved' => '#166534', 'validated' => '#166534', 'verified' => '#166534',
    'rejected' => '#991b1b', 'cancelled' => '#991b1b',
    'adjusted' => '#1e40af',
    'completed' => '#065f46',
    'in progress' => '#5b21b6', 'in_progress' => '#5b21b6',
];
$status_bg = [
    'pending' => '#fef3c7', 'pending validation' => '#fef3c7',
    'approved' => '#dcfce7', 'validated' => '#dcfce7', 'verified' => '#dcfce7',
    'rejected' => '#fee2e2', 'cancelled' => '#fee2e2',
    'adjusted' => '#dbeafe',
    'completed' => '#d1fae5',
    'in progress' => '#ede9fe', 'in_progress' => '#ede9fe',
];
...
foreach ($rows as $r) {
    ...
    $tbody .= '<tr>
        ...
        <td style="max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' . htmlspecialchars($r['service_description'] ?? '—') . '</td>
        ...
        <td class="tr"><strong>&#8369;' . number_format($cost, 2) . '</strong></td>
        <td>' . htmlspecialchars($r['staff_name'] ?? '—') . '</td>
        <td>' . htmlspecialchars($r['mechanic_name'] ?? '—') . '</td>
        <td style="white-space:nowrap;font-size:10px;">' . htmlspecialchars($r['created_at'] ?? '—') . '</td>
    </tr>';
}
...
foreach ($counts as $c) {
    ...
    $sb = $status_bg[$s]     ?? '#f1f5f9';
    $summary_rows .= '<tr>
        <td><span style="background:' . $sb . ';color:' . $sc . ';padding:2px 8px;border-radius:10px;font-size:11px;font-weight:600;">' . htmlspecialchars($c['status']) . '</span></td>
        <td class="tr">' . (int)$c['cnt'] . '</td>
        <td class="tr"><strong>&#8369;' . number_format((float)$c['total_cost'], 2) . '</strong></td>
    </tr>';
}
...
<div class="sec">&#128202; Status Summary</div>
<table style="max-width:400px;">
  <thead><tr><th>Status</th><th class="tr">Count</th><th class="tr">Total Cost</th></tr></thead>
  <tbody>' . $summary_rows . '</tbody>
</table>

<div class="sec">&#128203; Job Orders Detail</div>
' . ($rows ? '
<table>
  <thead><tr>
    <th>#</th><th>Customer</th><th>Service Type</th><th>Description</th>
    <th>Status</th><th class="tr">Cost</th><th>Staff</th><th>Mechanic / Tech</th><th>Date &amp; Time</th>
  </tr></thead>
  <tbody>' . $tbody . '</tbody>
  <tfoot><tr>
