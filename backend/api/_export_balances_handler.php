<?php
/**
 * Customer Balances Export Handler
 * Columns: Customer Name/ID, Outstanding Balance, Credit Limit, Usage %, Due Date, Status, Remarks
 * Expects: $pdo, $station_id, $_GET['format']
 */
...
catch (Exception $e) { try { $pdo->query("SELECT customer_id FROM merchandise_transactions LIMIT 0"); } catch (Exception $e2) { $mt_cust_col = null; } }
...
$filename = 'customer_balances_' . date('Y-m-d');
...
if ($fmt === 'csv') {
    ...
    fputcsv($out, ['Customer Balances Report']);
    fputcsv($out, ['Station:', $station_name]);
    fputcsv($out, ['Generated:', date('Y-m-d H:i:s')]);
    fputcsv($out, []);

    fputcsv($out, ['Customer ID', 'Customer Name', 'Outstanding Balance (PHP)', 'Credit Limit (PHP)', 'Usage %', 'Due Date', 'Status', 'Remarks']);
    $tot_bal = 0; $tot_lim = 0;
    foreach ($rows as $r) {
        $bal = (float)$r['outstanding_balance'];
        $lim = (float)$r['credit_limit'];
        $tot_bal += $bal; $tot_lim += $lim;
        $remarks = $r['remarks'] ?: ($bal > $lim && $lim > 0 ? 'Exceeded credit limit' : ($r['payment_status'] === 'Overdue' ? 'Payment past due date' : '—'));
        fputcsv($out, [
            '#' . $r['id'],
            $r['customer_name'],
            number_format($bal, 2),
            $lim > 0 ? number_format($lim, 2) : 'No limit',
            $r['usage_pct'] . '%',
            $r['due_date'] ?? '—',
            $r['payment_status'],
            $remarks,
    ...
    if ($rows) {
        fputcsv($out, ['', 'TOTAL', number_format($tot_bal, 2), number_format($tot_lim, 2), '', '', '', '']);
    }
    fclose($out);
    exit;
}
...
$status_styles = [
    ...
    'over limit' => ['bg'=>'#fef3c7','color'=>'#92400e'],
...
foreach ($rows as $r) {
    ...
    $status = $r['payment_status'] ?? 'Active';
    $tot_bal += $bal; $tot_lim += $lim;
    $ss = $status_styles[strtolower($status)] ?? ['bg'=>'#f1f5f9','color'=>'#475569'];
    $bar_color = $pct >= 100 ? '#dc2626' : ($pct >= 80 ? '#d97706' : '#002F6C');
    $bar_w = min($pct, 100);
    $remarks = $r['remarks'] ?: ($bal > $lim && $lim > 0 ? 'Exceeded credit limit' : ($status === 'Overdue' ? 'Payment past due date' : '—'));
    $tbody .= '<tr>
        <td>
            <strong>' . htmlspecialchars($r['customer_name'] ?? '—') . '</strong>
            <div style="font-size:9px;color:#94a3b8;">#' . (int)$r['id'] . '</div>
        </td>
        <td class="tr" style="color:' . ($bal>0?'#dc2626':'#16a34a') . ';font-weight:700;">&#8369;' . number_format($bal, 2) . '</td>
        <td class="tr">' . ($lim > 0 ? '&#8369;' . number_format($lim, 2) : '<span style="color:#94a3b8;">No limit</span>') . '</td>
        <td>' . ($lim > 0
            ? '<div style="display:flex;align-items:center;gap:5px;">
                 ...
                 <span style="font-size:9px;font-weight:600;color:' . $bar_color . '">' . $pct . '%</span>
               </div>'
            : '<span style="color:#94a3b8;">—</span>') . '</td>
        <td style="' . ($status==='Overdue'?'color:#dc2626;font-weight:600;':'') . '">' . htmlspecialchars($r['due_date'] ?? '—') . '</td>
        <td><span style="background:' . $ss['bg'] . ';color:' . $ss['color'] . ';padding:2px 7px;border-radius:10px;font-size:10px;font-weight:600;">' . htmlspecialchars($status) . '</span></td>
        <td style="font-size:10px;color:#64748b;">' . htmlspecialchars(mb_strimwidth($remarks, 0, 50, '…')) . '</td>
    </tr>';
}
...
<div class="sec">&#128179; Customer Balances</div>
' . ($rows ? '
<table>
  <thead><tr>
  ...
  <tbody>' . $tbody . '</tbody>
  <tfoot><tr>
    <td><strong>TOTAL (' . count($rows) . ' customers)</strong></td>
    <td class="tr"><strong>&#8369;' . number_format($tot_bal, 2) . '</strong></td>
    <td class="tr"><strong>&#8369;' . number_format($tot_lim, 2) . '</strong></td>
    <td colspan="4"></td>
  </tr></tfoot>
