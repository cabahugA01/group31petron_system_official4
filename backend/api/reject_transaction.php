...
try {
    ...
    if ($fuel_row) {
        ...
        $stmt = $pdo->prepare("UPDATE merchandise_transactions SET validation_status = 'Rejected', validated_by = ?, validated_at = NOW(), rejection_reason = ?, updated_at = NOW() WHERE id = ? AND station_id = ?");
    ...
    try {
        $pdo->prepare("INSERT INTO audit_trail (transaction_id, manager_id, action_type, new_value, station_id) VALUES (?, ?, 'Return', ?, ?)")
            ->execute([$db_txn_id, $manager_id, $reason, $station_id]);
    } catch (Exception $ae) {}
