<?php
// Bypass login check for this API endpoint
$_SERVER['SCRIPT_NAME'] = '/public/update_prices.php'; // Trick to bypass require_login check
...
try {
    ...
    foreach ($missing_items as $item) {
        ...
        $check_stmt = $pdo->prepare("SELECT id FROM products WHERE LOWER(TRIM(name)) = LOWER(TRIM(?))");
        $check_stmt->execute([$item['name']]);
        $existing = $check_stmt->fetch(PDO::FETCH_ASSOC);
        
        if (!$existing) {
            // Insert new item without category_id to avoid foreign key issues
            ...
            // Update existing item with proper prices
            $update_stmt = $pdo->prepare("
                UPDATE products SET cost = ?, price = ?, updated_at = NOW() 
                WHERE LOWER(TRIM(name)) = LOWER(TRIM(?))
            ");
            $update_stmt->execute([$item['cost'], $item['price'], $item['name']]);
        }
    }
    ...
    // Check for products with zero cost but non-zero price
    $stmt = $pdo->query("SELECT id, name, cost, price FROM products WHERE cost = 0.00 AND price > 0.00");
    $zero_cost_products = $stmt->fetchAll(PDO::FETCH_ASSOC);
    
    if (!empty($zero_cost_products)) {
        foreach ($zero_cost_products as $product) {
            // Set cost to 80% of price as a reasonable default
            $default_cost = $product['price'] * 0.8;
            
            $update_stmt = $pdo->prepare("UPDATE products SET cost = ? WHERE id = ?");
            $update_stmt->execute([$default_cost, $product['id']]);
