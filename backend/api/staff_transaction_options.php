...
try {
    ...
    try {
        $stmt = $pdo->prepare("
            SELECT
                ...
                COALESCE(fi.price_per_liter, si.price, si.cost, ip.unit_cost, 0) AS price_per_liter
        ...
        $debug['messages'][] = 'Fuel query failed: ' . $e->getMessage();
    }
        ...
        try {
            $stmt = $pdo->prepare("
                SELECT
                    ...
                    0 AS calibration,
                    COALESCE(si.price, si.cost, ip.unit_cost, 0) AS price_per_liter
            ...
            $debug['messages'][] = 'Fuel fallback query failed: ' . $e->getMessage();
        }
    }
    ...
    try {
        ...
        ");
        $stmt->execute([$station_id]);
        $merchandise_options = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];
    } catch (Throwable $e) {
        $debug['messages'][] = 'Merchandise query failed: ' . $e->getMessage();
    }
    ...
    if (!$merchandise_options) {
        try {
            ...
            $debug['messages'][] = 'Merchandise fallback query failed: ' . $e->getMessage();
        }
    }

    $debug['fuel_count'] = count($fuel_options);
    $debug['merchandise_count'] = count($merchandise_options);

    if (!$fuel_options) {
        $debug['messages'][] = 'No fuel options found for the current station.';
    }

    if (!$merchandise_options) {
        $debug['messages'][] = 'No merchandise options found for the current station.';
    }

    echo json_encode([
        'success' => true,
        'fuel_options' => $fuel_options,
        'merchandise_options' => $merchandise_options,
        'debug' => $debug,
    ]);
} catch (Throwable $e) {
