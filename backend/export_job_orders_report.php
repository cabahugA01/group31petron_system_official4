<?php
/**
 * Job Orders Report Export Backend
 * Handles advanced export with custom date ranges and filters
 */
...
try {
    ...
    $sql = "SELECT 
                ...
                jo.created_at,
                u.name AS staff_encoder
            FROM job_orders jo
            LEFT JOIN customers c ON jo.customer_id = c.id
            LEFT JOIN users u ON jo.created_by = u.user_id
            WHERE jo.station_id = ? 
    ...
    if ($export_type === 'json') {
        ...
        echo json_encode([
            ...
            'total_records' => count($job_orders)
        ]);
    }
