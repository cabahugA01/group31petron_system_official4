...
try {
    // Fetch job orders created by this staff member
    $stmt = $pdo->prepare("
        SELECT 
            COALESCE(jo.job_order_id, jo.job_order_number, CONCAT('JO-', jo.id)) AS jo_id,
            COALESCE(jo.customer_name, 'Walk-in') AS customer,
            COALESCE(jo.service_type, jo.service_description, 'N/A') AS service,
            ...
            DATE_FORMAT(jo.created_at, '%Y-%m-%d %H:%i') AS date_created
        FROM job_orders jo
        WHERE jo.created_by = ?
    ...
    if ($format === 'csv') {
        ...
        header('Content-Disposition: attachment; filename="job_orders_' . date('Y-m-d') . '.csv"');
        
        $output = fopen('php://output', 'w');
        fputcsv($output, ['Job Order ID', 'Customer', 'Service', 'Vehicle', 'Status', 'Amount', 'Payment Method', 'Date Created']);
        ...
        header('Content-Disposition: attachment; filename="job_orders_' . date('Y-m-d') . '.xls"');
        
        echo '<table border="1"><thead><tr>';
        echo '<th>Job Order ID</th><th>Customer</th><th>Service</th><th>Vehicle</th><th>Status</th><th>Amount</th><th>Payment Method</th><th>Date Created</th>';
        echo '</tr></thead><tbody>';
