...
if ($found) {  echo "Found  : YES  (id={$found['id']}, status={$found['status']})\n";
} else {  echo "Found  : NO  — email not found by LOWER(TRIM()) match\n";
}
...
foreach ($likes as $l) {  echo "  id={$l['id']} user={$l['username']} email='{$l['email']}'\n";
}
