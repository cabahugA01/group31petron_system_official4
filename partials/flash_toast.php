...
#petron-toast.toast-success {
    background: #16a34a;
    color: #fff;
    border: 1.5px solid #15803d;
}
#petron-toast.toast-error {
    background: #dc2626;
    color: #fff;
...
<?php if ($_ft_success): ?>
<div id="petron-toast" class="toast-success" role="alert">
    <span class="toast-icon"><i class="fas fa-check-circle"></i></span>
    <span class="toast-msg"><?= htmlspecialchars($_ft_success) ?></span>
    <button class="toast-close" onclick="closePetronToast()" title="Close">&times;</button>
</div>
<?php elseif ($_ft_error): ?>
<div id="petron-toast" class="toast-error" role="alert">
    <span class="toast-icon"><i class="fas fa-exclamation-circle"></i></span>
    <span class="toast-msg"><?= htmlspecialchars($_ft_error) ?></span>
    <button class="toast-close" onclick="closePetronToast()" title="Close">&times;</button>
</div>
<?php endif; ?>
...
<script>
(function() {
    var t = document.getElementById('petron-toast');
    if (!t) return;
    // Slide in after a tiny delay so transition fires
    setTimeout(function() { t.classList.add('show'); }, 60);
    // Auto-dismiss after 5 seconds
    var autoHide = setTimeout(function() { closePetronToast(); }, 5500);
    window.closePetronToast = function() {
