...
foreach (['transaction_adjustment', 'delivery_adjustment'] as $adj_type) {
    ...
    echo "This Month: " . $sp3->fetchColumn() . "\n";
    ...
    $stmt->execute([$station_id, $adj_type, date('Y-m-d', strtotime('-30 days')), date('Y-m-d')]);
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    echo "Rows in last 30 days: " . count($rows) . "\n";
    foreach ($rows as $r) {
        $nd = json_decode($r['notes'], true) ?: [];
        echo "  ADJ-{$r['id']} | {$r['fuel_type']} | Prev:{$r['previous_value']} -> New:{$r['new_value']} | By:{$r['manager_name']}\n";
