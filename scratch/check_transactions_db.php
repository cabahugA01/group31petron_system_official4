...
try {
    $sql = "
        SELECT
            ...
            mt.amount_paid AS amount_paid,
            COALESCE(mt.payment_method,'Cash') AS payment_method,
            COALESCE(NULLIF(mt.transaction_date, '0000-00-00'), mt.created_at) AS txn_date,
            COALESCE(mt.validation_status,'Approved') AS validation_status,
        ...
        WHERE mt.station_id = ? AND DATE(COALESCE(NULLIF(mt.transaction_date, '0000-00-00'), mt.created_at)) >= ? AND DATE(COALESCE(NULLIF(mt.transaction_date, '0000-00-00'), mt.created_at)) <= ?
        GROUP BY mt.id
    ...
    foreach ($rows as $r) {
        echo "Row ID: " . $r['row_id'] . " | Txn ID: " . $r['txn_id'] . " | Customer: " . $r['customer'] . " | Type: " . $r['entry_type'] . " | Amount: " . $r['amount'] . " | Validation Status: " . $r['validation_status'] . "\n";
    }
} catch (Exception $e) {
