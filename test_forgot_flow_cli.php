...
if (empty($users)) {
    echo "No active users with email found!\n";
    exit(1);
}

foreach ($users as $u) {
    echo sprintf("  [%d] %-30s %-20s %-15s %s\n", $u['id'], $u['name'], $u['username'], $u['role'], $u['email']);
}
...
foreach ($users as $u) {
    ...
    echo "\n[{$u['id']}] {$u['name']} ({$u['role']}) → sending OTP {$otp} to: {$email} ... ";
    ...
    $pdo->prepare("INSERT INTO password_reset_tokens (user_id, token, token_type, expires_at, ip_address) VALUES (?, ?, 'reset', DATE_ADD(NOW(), INTERVAL 5 MINUTE), ?)")
        ->execute([$u['id'], $otp, '127.0.0.1']);

    // Send email
    $sent = sendPasswordResetOTP($email, $otp);
    echo $sent ? "✓ SENT\n" : "✗ FAILED\n";
}
