...
echo "Current User: " . ($me['username'] ?? 'unknown') . "\n";
...
try {
    ...
    echo "Query returned " . count($items) . " items (showing first 10)\n\n";
    
    foreach ($items as $item) {
        echo "- {$item['name']} | Category: {$item['category_name']} | Status: {$item['status']} | Stock: {$item['stock_level']}\n";
    }
