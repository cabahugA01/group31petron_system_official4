<?php
/**
 * Final Verification - Confirm all 101 services are properly loaded
 */
...
if ($total != 101) {
    echo "⚠ WARNING: Expected 101 services, found {$total}\n\n";
}
...
foreach($categories as $c) {
    $count = str_pad($c['count'], 3, ' ', STR_PAD_LEFT);
    echo "  {$count} │ {$c['category']}\n";
}

echo "\n═══ SAMPLE SERVICES (First 10) ═══\n";
$samples = $pdo->query('
    SELECT category, service_name, service_price 
    FROM job_order_service_types 
...
foreach($samples as $s) {
    $price = '₱' . number_format($s['service_price'], 2);
    echo "  • {$s['category']}: {$s['service_name']} - {$price}\n";
}
...
$priceStats = $pdo->query('
    SELECT 
        ...
        COUNT(CASE WHEN service_price = 0 THEN 1 END) as free_count,
        COUNT(CASE WHEN service_price > 0 AND service_price <= 500 THEN 1 END) as under_500,
        COUNT(CASE WHEN service_price > 500 AND service_price <= 2000 THEN 1 END) as range_500_2000,
        COUNT(CASE WHEN service_price > 2000 AND service_price <= 5000 THEN 1 END) as range_2000_5000,
        COUNT(CASE WHEN service_price > 5000 THEN 1 END) as over_5000
    FROM job_order_service_types
')->fetch(PDO::FETCH_ASSOC);

echo "  Min Price:      ₱" . number_format($priceStats['min_price'], 2) . "\n";
